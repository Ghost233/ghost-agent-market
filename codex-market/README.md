...
Owner 是稳定的逻辑责任域，Agent 只是软亲和执行载体。runtime 为 spawn 下发 canonical `executor_spawn_name`，协调器不得自行生成；正确性只依赖持久状态，不依赖 Agent 会话记忆。执行模式固定为 `subagent`，Codex 子代理固定使用 `gpt-5.6-sol/medium`，不同 Goal 不复用 Agent。
