...
`git-commit` 先使用固定为 `gpt-5.3-codex-spark/high` 的只读 `git_commit_worker` 分析当前 checkout；只有工具、系统或契约失败时才创建一个 `gpt-5.6-luna/medium` 只读分析线程作为 fallback，再由主线程复核并执行实际 Git 提交。
