...
独立 `ghost-agent-skills` 插件中的 `git-commit` 固定使用 `gpt-5.6-terra/medium` 的执行子代理完成整个提交事务；执行子代理再启动同配置且不 fork 上下文的只读子代理独立审查，主线程只调度和转发结果。它不依赖 Goal、Owner、DAG 或子线程工作流。`rtk-hook` 对未通过 `rtk` 前缀执行的 shell 命令给出重试提示。
