...
独立 `ghost-agent-skills` 插件中的 `ghost-implement-spec` 复用 Matt 的 `implement-spec`，只覆盖全部子代理为 `gpt-5.6-terra/xhigh`；`git-commit` 在单个隔离 executor 中检查改动，并通过 Python 3 安全脚本创建规范的中文 Git 提交；`git-merge-conflict` 在修改冲突文件前先用只读 Bash 脚本锁定 merge/rebase/cherry-pick 三方和有界历史，再按考古证据解决高风险冲突。`rtk-hook` 通过 `rtk rewrite` 透明改写 RTK 支持的 shell 命令，不支持的命令原样放行，也不再阻断后要求重试。
