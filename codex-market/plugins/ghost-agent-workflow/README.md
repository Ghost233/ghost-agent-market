...
默认使用 `standalone_thread`，不要求原生 `/goal`。启动前必须由用户明确选择模式；Quick 由 Main 串行执行 Owner 与显式 Review；DAG 移交后的 Main 使用 `gpt-5.6-sol/xhigh`，Supervisor 使用 `gpt-5.6-luna/medium`，最多并发 8 个 ready 线程。配置包含五组 profile，机械 gate 与定向验证由脚本执行。
