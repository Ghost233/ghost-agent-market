...
Codex 默认 module `worker_profile` 为 `gpt-5.6-terra/xhigh`。coordinator 不从默认值补 module 缺失字段，只转换计划中已经解析完整的值：

1. `worker_profile.model` 必须是当前实现子代理接口可识别的完整 model id，并原样用于调度参数 `model`。默认值为 `gpt-5.6-terra`；不要猜 alias、近似模型或回退值。
...
不要读取或复用历史 thread profile。为每个 module 在调用前生成唯一 dispatch request id，并在同一次原子 implementation-subagent 调用中同时传入 `model`、`thinking` 和完整分派包。只有接口接受参数时 subagent 才会收到包，因此包内 evidence 可以记录该 request id 和原子调用参数；调用返回的 subagent id 由 coordinator 在调用完成后单独保存，不是首包前置字段：
...
worker_profile_evidence:
  ...
  evidence: <coordinator dispatch request id、原子调用参数或拒绝原因>
```

`requested` 必须逐字段等于 plan-authored `worker_profile`；`dispatch_arguments` 必须等于确定性映射结果。原子调用请求只有在接口接受参数并交付首包时才视为 `applied`；接口拒绝时不会启动 worker，coordinator 记录 `rejected`。`applied` 是分派和完成的必要条件。`unavailable` 或 `rejected` 时不启动替代 worker、不静默降级、不消耗补修次数；直接把该 module 和受其依赖的后续 module 标为 blocked。

本契约要求新版 Codex implementation-subagent 接口支持 `model`、`thinking` 和初始 prompt。当前客户端的子代理 schema 缺少任一参数时返回 `dispatch_unavailable` 并提示升级；不得改用用户可见 thread/task 绕过能力门禁。
