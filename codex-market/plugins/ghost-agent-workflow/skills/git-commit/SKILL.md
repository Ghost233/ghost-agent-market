...
6. `create_thread` 返回 thread id 后执行最多三轮启动确认：第一轮立即调用 `read_thread`，不得等待；只有第 8 步判定状态尚不明确时，第二、第三轮才各先等待 10 秒再调用 `read_thread`。每轮读取后，尚未命名成功时调用一次 `set_thread_title`，然后立即按第 8 步判断。命名成功后后续轮次不得重复改名；单次读取或命名失败只记录原始错误。
...
8. 每轮读取后立即处理：
   ...
   - 状态仍在运行但没有执行线程记录、读取失败或暂时找不到线程：下一轮等待 10 秒后重试，直到最多三轮。
