...
```json
{
  ...
  "owners": [
    {
      ...
      "runtime_profile": {"model": "gpt-5.6-sol", "reasoning_effort": "medium"},
  ...
  "tasks": [
    ...
    },
    {
      ...
      "title": "验证 Goal 执行流程",
      "role": "verify",
      "owner_id": "runtime-verification",
      "task": "只读运行完整 Goal DAG smoke",
      "depends_on": ["T1"],
      "writable_paths": [],
...
每个 Codex Owner 的 profile 必须精确等于 `gpt-5.6-sol/medium`。每个 work task 必须依赖当前 `source-coverage-audit`；所有 task 都必须有非空 `plan_item_ids` 和合法 `coverage_effect`。
