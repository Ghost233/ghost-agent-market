# Codex Coverage、Plan 与 Delta 模板
  ...
  "owners": [
    {
      ...
      "runtime_profile": {"model": "gpt-5.6-sol", "reasoning_effort": "high"},
...
模块 Owner 必须逐字来自 approved Registry，`writable_paths`/`excluded_paths` 对应 Registry 的 include/exclude；同一模块的 implementation/verification/review/research 复用同一 Owner。三个固定机械 actor 必须且只能放在 `runtime_actors[]`，task 以 `owner_id: null + runtime_actor_id` 引用；模块 task 则反过来。每个 Codex 执行条目的 profile 必须精确等于 `gpt-5.6-sol/high`。每个 work task 必须依赖当前 `source-coverage-audit`；最终链路依次包含 Owner commit attestation、`diff-audit` 与 `commit-readiness`。所有 task 都必须有非空 `plan_item_ids` 和合法 `coverage_effect`。
