...
收到预备提示时，只确认 `task_id`、`logical_id` 和 `module_id`，然后等待包含真实 `thread_id` 的完整绑定包。此前不创建目标、不读取或修改实现文件、不运行命令。
...
3. `task_id` 和 `logical_id` 在计划中唯一；绑定字段与计划中的任务逐字段一致；`module_id` 指向存在的模块。
...
首次执行任务时，创建绑定 `task_id`、`logical_id`、`module_id` 和 `writable_paths` 的独立目标，并在编辑前再次核对。补修时恢复当前目标，不为同一 `logical_id` 创建第二个并行目标。
...
`completed` 必须满足：计划绑定校验通过；模型配置证据可核对；changed files 全部在写入范围内；`done_when` 已满足；验证通过或有明确替代证据；差异自检为 `pass`；不存在未解决依赖、共享文件冲突或用户干预。
