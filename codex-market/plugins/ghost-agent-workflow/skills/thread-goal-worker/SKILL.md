...
description: 仅供 thread-coordination 分发完整 TASK_BINDING_V4 时内部使用，且 binding.executor_mode 必须为 thread；执行同一 Goal/Owner 的一个 fenced attempt，从 Owner Capsule 恢复上下文并写入唯一 WORKER_RESULT_V4。普通请求、不完整绑定、其他 mode 或绕过 coordinator 的调用不得触发。
---
...
只执行一个 binding。逻辑 Owner 与 Capsule 是持久真相源；当前线程的会话记忆和复用只是性能优化。

不得调用 `create_thread`、`spawn_agent`、`fork_thread` 或继续委派。不得修改 goal/coverage/plan/state/capsule，不得直接编辑 `capsule.json`，不得暂存、提交、推送或扩大 Goal。协调元数据只允许写 binding 指定的 checkpoint_path、attempt 唯一 result_path，以及固定 audit binding 中非空的精确 `evidence_artifact_paths`（source proposal/runtime audit artifact）；其它 runtime 路径不可写。
...
4. 任一身份或路径不一致时不修改业务文件，返回原始错误。同一 task/attempt/token 的完全相同 binding 是 crash recovery 重投：不得重置进度、并发重复执行或覆盖已有 canonical result；继续当前 attempt，结果已存在时只返回同一结果。只有 state 已进入新 attempt 且新 binding 完整通过门禁时，才替换旧 task 权限。同一线程同时只能执行一个 binding。
5. 新 generation 线程从 Capsule 的 decisions、invariants、progress、important_symbols 与 next_steps 恢复，不依赖旧线程存活。
