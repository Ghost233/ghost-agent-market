...
初始 plan 为 `revision: 1`，continuation 必须比直接前版加一。驱动器要求 `reviewed_task_ids/replacements` 覆盖全部未完成旧 task，且旧 plan 没有 running。它读取旧 plan/state，验证 parent goal、完整 module 定义、终止状态和真实 thread id。`continue` 要求新旧 `logical_id` 相同；`handoff` 只允许 completed 来源且 logical id 必须改变。同一个旧线程最多映射一个当前 task。
...
7. 持久化 running 后，把标题设为 `[执行] <logical_id> · <title>`；终止后更新为 `[完成|复核|阻塞|失败]`，但命名失败不影响执行。
...
10. continuation 只复用同父目标、同 module、已终止且有真实 thread id 的旧任务。
11. action 带稳定 `logical_id` 和可读 `title`，协调器不会留下通用“等待绑定包”标题。
12. 插件 manifest 合法，基础版本按仓库规则递增，发布后 `.mjs` 在无项目依赖时可执行。
