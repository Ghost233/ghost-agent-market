...
每个 ready module 使用一次 thread 创建调用：
...
create_thread(
  ...
  prompt=<single-module dispatch package>
)
```

同一 batch 的 ready module 可以并发创建。创建成功后，coordinator 保存真实 thread id，并为 UI 输出对应的 created-thread 记录。
