...
为 `$parallel-task-planner`、`$thread-coordination` 和 `$thread-goal-worker` 增加统一的 worker 模型配置契约。用户可以为并发任务设置默认模型与思考强度，并按 module 覆盖；规划器解析配置，协调器负责实际调度和证据校验，worker 只确认自己收到的有效配置。

本次是对外可见的新能力，Codex 插件基础版本从 `0.2.0` 升至 `0.3.0`。发布时额外加上 `+codex.<UTC 时间戳>` cachebuster，使已安装客户端在重新安装时明确识别为新构建。

## 配置契约

规划器接受一个可选默认 profile；每个 module 可以覆盖其中任一字段：
...
coordinator 收到 `parallel_safe` 计划时，逐 module 读取完整 `worker_profile`。在分派前，它必须通过当前 Codex thread 创建/复用接口验证模型和思考强度可以真正应用：
...
worker_profile:
  ...
  evidence: "create_thread 返回 model=gpt-5.3-codex-spark、reasoning_effort=xhigh"
```
...
worker 将 `worker_profile` 当作不可变的派发约束。它在设置 `/goal` 前检查 profile 字段完整性，并在最终 `WORKER_RESULT` 或 `COORDINATOR_RESULT` 中回报请求 profile、已确认 effective profile、状态和证据。

worker 不自行选择、切换或覆盖模型及思考强度；若运行时无法确认它实际使用了所请求的 profile，停止在 `needs_main_review` 或 `blocked`，而不是把普通文本里的模型声明当作成功证据。
...
| 调度接口无法应用 profile | 不假定成功 | `blocked` | 不伪称 applied |
...
3. 对“默认值 + module 覆盖”“缺少 profile”“复用 profile 不匹配”“运行时不支持设置 profile”四类压力场景进行前向测试。
