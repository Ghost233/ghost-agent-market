...
`/goal` 由 Codex 自身创建并持久化 objective。objective 中的“每轮使用 `$subagent-coordination`”确保每个外循环轮次都显式载入控制器；不是由插件解析或模拟 `/goal`。
...
```text
/ghost-agent-workflow:subagent-coordination 执行 `./plan.md`，以子代理 DAG 完整执行，直到计划项覆盖率 100% 且所有验收通过。
```
...
| 推荐首次入口 | `/goal 每轮使用 $subagent-coordination，…` | `/ghost-agent-workflow:subagent-coordination 执行 …` |
| 每轮 skill 触发 | 原生 objective 明确要求每轮显式 `$subagent-coordination` | namespaced skill 命令显式触发 |
