...
**Architecture:** `$parallel-task-planner` 是唯一计划生产者，输出 `plan_format_version: 1`、平台和完整 module profiles；`$thread-coordination` 只接受绝对 `plan_path` 并验证计划、运行时 profile 与安全证据；`$thread-goal-worker` 只接受 coordinator 发出的单 module 包并回报 profile evidence。Claude Code 与 Codex 保留各自执行面差异，但共享相同字段和拒绝旁路规则。
...
让 fresh subagent 使用当前 `$parallel-task-planner`，为 Claude Code 并发任务生成计划，检查是否包含 `planner`、`plan_format_version`、`execution_platform`、完整 `worker_profile` 与 `reviewer_subagent_profile`。
...
- Produces: 顶层 `planner: parallel-task-planner`、`plan_format_version: 1`、`execution_platform`，以及每个 module 完整的 `worker_profile` 和 `reviewer_subagent_profile`。
...
加入版本化计划来源、平台默认 profile、module 覆盖解析、缺失 profile 时 `needs_user_review`、禁止静默降级，以及只对 `parallel_safe` 计划自动交接的规则。
...
**Interfaces:**
- Consumes: planner 生成的绝对 `plan_path`、`plan_format_version: 1`、`execution_platform` 和 `safety.status: parallel_safe`。
- Produces: 逐 module profile evidence、`WORKER_RESULT` 汇总和 `PARALLEL_PLAN_RESULT`；非法入口只返回 `blocked`。
...
只有计划同时包含以下内容才允许分派：`planner: parallel-task-planner`、`plan_format_version: 1`、匹配当前平台的 `execution_platform`、完整 modules、`dispatch.batches`、`safety.status: parallel_safe`、无当前工作区冲突。

- [ ] **Step 3: 加入平台 profile 预检与证据**

Codex 使用 `sol/xhigh -> terra/xhigh`；Claude Code 使用 `opus/max -> sonnet/max`。复用或创建 thread/teammate 前验证 profile，无法应用、读取或匹配时停止；结果使用 `requested`、`effective`、`status`、`evidence` 四个字段。
...
**Interfaces:**
- Consumes: coordinator 派发的单 module 包，包含 planner 来源链、两个 profiles、scope、done_when 和 verification。
- Produces: `WORKER_RESULT`，包含 worker 与 reviewer profile evidence；普通分派返回 `blocked`。

- [ ] **Step 1: 收紧输入契约**

把 `planner: parallel-task-planner`、`plan_format_version: 1`、`execution_platform`、`module_id`、`worker_profile`、`reviewer_subagent_profile` 设为执行前必需字段。缺少任一字段时不设置 goal、不修改文件。

- [ ] **Step 2: 加入 profile 证据与 reviewer 规则**

worker 不允许自行切换 profile；只有运行时 evidence 匹配请求值才执行。普通修改型任务的 reviewer 必须匹配平台默认 profile；`parallel-plan` diff 自检例外写 `not_required`，不能伪造 reviewer evidence。

- [ ] **Step 3: 更新结果契约**

在 `WORKER_RESULT` 和必要的 blocked 返回中加入 worker/reviewer 的 `requested`、`effective`、`status`、`evidence`；只有必需 profile 为 `applied` 才允许 `completed`。
...
Expected: 无输出。再检查六份 skill 都包含 `planner`、`plan_format_version`、`execution_platform`、`worker_profile`、`reviewer_subagent_profile`，且平台默认值符合 Global Constraints。
