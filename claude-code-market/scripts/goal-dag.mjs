...
function writeTextDurable(path        , payload        )       {
...
function parseRegistryWorktreeBinding(value         , ownerLabel        )                          {
  ...
  return {
    ...
    committed_at: requireNullableString(source.committed_at, `${ownerLabel}.committed_at`),
    merged_oid: oid("merged_oid"),
    merged_at: requireNullableString(source.merged_at, `${ownerLabel}.merged_at`),
  };
}
...
function assertOwnerDependencyGraph(owners                 , label        )       {
  ...
  function visit(owner               )       {
    ...
    for (const dependencyId of owner.depends_on_owners) {
      if (dependencyId === owner.owner_id) fail(`${label}: owner ${owner.owner_id} cannot depend on itself`);
      const dependency = byId.get(dependencyId);
      if (dependency === undefined) fail(`${label}: owner ${owner.owner_id} depends on unknown owner ${dependencyId}`);
      if (owner.lifecycle === "active" && dependency.lifecycle !== "active") {
        fail(`${label}: active owner ${owner.owner_id} depends on inactive owner ${dependencyId}`);
      }
      visit(dependency);
    }
    visiting.delete(owner.owner_id);
    visited.add(owner.owner_id);
  }
...
function parseOwnerRegistry(value         )                 {
  ...
  assertRegistryDisjoint(owners.filter((owner) => owner.lifecycle === "active"), "registry");
  assertOwnerDependencyGraph(owners, "registry");
  return {
...
function parseOwnerDelivery(value         , goal              )                       {
  ...
  const registryRef = requireString(source.registry_ref, "owner_delivery.registry_ref");
  const registryDigest = requireString(source.registry_digest, "owner_delivery.registry_digest");
  if (!isAbsolute(registryRef) || !/^[0-9a-f]{64}$/u.test(registryDigest)) fail("owner_delivery registry binding is invalid");
  const featureBranch = requireString(source.feature_branch, "owner_delivery.feature_branch");
  const baseOid = requireString(source.base_oid, "owner_delivery.base_oid");
  if (!/^[0-9a-f]{40,64}$/u.test(baseOid)) fail("owner_delivery.base_oid is invalid");
  ...
  for (const ownerId of requiredOwnerIds) {
    ...
    owners[ownerId] = {
      owner_id: ownerId, worktree_path: typeof item.worktree_path === "string" ? item.worktree_path : "",
  ...
  return { contract: "OWNER_DELIVERY_V1", registry_ref: resolve(registryRef), registry_digest: registryDigest,
    feature_branch: featureBranch, base_oid: baseOid, required_owner_ids: requiredOwnerIds, owners,
    pending_intent: source.pending_intent === null ? null : requireRecord(source.pending_intent, "owner_delivery.pending_intent"),
    frozen_evidence: source.frozen_evidence === null ? null : requireRecord(source.frozen_evidence, "owner_delivery.frozen_evidence") };
}
...
function parseGoalState(
  ...
  return {
    ...
    owner_delivery: parseOwnerDelivery(source.owner_delivery, goal),
  };
}
...
function taskBinding(
  ...
  return {
    ...
    owner_exec: (() => {
      if (task.role !== "work" || goalState.owner_delivery === null) return null;
      const entry = goalState.owner_delivery.owners[owner.id];
      if (entry === undefined || entry.status === "pending" || !entry.worktree_path || !entry.owner_branch) {
        fail(`owner delivery worktree is not ready for owner ${owner.id}`);
      }
      const registry = parseOwnerRegistry(readJson(goalState.owner_delivery.registry_ref));
      const registryOwner = findActiveOwner(registry, owner.id);
      const binding = registryOwner.worktree_binding;
      if (digestFile(goalState.owner_delivery.registry_ref) !== goalState.owner_delivery.registry_digest ||
          binding === null || binding.worktree_path !== entry.worktree_path || binding.owner_branch !== entry.owner_branch ||
          binding.base_oid !== entry.base_oid || binding.feature_branch !== goalState.owner_delivery.feature_branch) {
        fail(`owner delivery binding drift for owner ${owner.id}`);
      }
      return { agent_type: `owner-${owner.id}`, worktree_path: entry.worktree_path,
        owner_branch: entry.owner_branch, base_oid: entry.base_oid, owned_modules_glob: entry.owned_modules_glob };
    })(),
  };
}
  ...
  const payload = withStateLock(statePath, () => {
    ...
    for (const task of ready) {
      ...
      actions.push({
        ...
        executor_spawn_name: ownerExec === null ? spawnName : ownerExec.agent_type,
        reservation_token: taskState.reservation_token,
...
function diffAuditCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    const goalState = goalStateForPlan(planPath, plan, goal).state;
    if (goalState.owner_delivery !== null && goalState.owner_delivery.required_owner_ids.some(
      (ownerId) => goalState.owner_delivery?.owners[ownerId].status !== "merged",
    )) fail(`${DIFF_SCOPE_GATE_ID} requires all owner deliveries merged`);
    const taskState = state.tasks[taskId];
    if (taskState.status !== "running") fail(`task ${taskId} is not running`);
    if (taskState.reservation_token !== reservationToken) fail("reservation token mismatch");
    if (taskState.source_revision !== state.source_revision) fail("source revision mismatch");
...
function finalizeCommand(
  ...
  const payload = withStateLock(goalStatePath, () => withStateLock(statePath, () => {
    ...
    if (goalState.owner_delivery !== null) {
      if (goalState.owner_delivery.pending_intent !== null) fail("owner delivery has pending intent");
      for (const ownerId of goalState.owner_delivery.required_owner_ids) {
        const entry = goalState.owner_delivery.owners[ownerId];
        if (entry.status !== "merged" || entry.committed_oid === null || entry.merged_oid === null) fail(`owner ${ownerId} must be committed and merged before finalize`);
        const committedAncestor = spawnSync("git", ["-C", goal.workspace.root, "merge-base", "--is-ancestor", entry.committed_oid, entry.merged_oid], { encoding: "utf8", shell: false });
        if (committedAncestor.status !== 0) fail(`owner ${ownerId} committed_oid is not contained by merged_oid`);
        const mergedAncestor = spawnSync("git", ["-C", goal.workspace.root, "merge-base", "--is-ancestor", entry.merged_oid, goalState.owner_delivery.feature_branch], { encoding: "utf8", shell: false });
        if (mergedAncestor.status !== 0) fail(`owner ${ownerId} merged_oid is not in feature branch`);
      }
    }
    ...
    if (goalState.owner_delivery !== null) goalState.owner_delivery.frozen_evidence = {
      registry_digest: goalState.owner_delivery.registry_digest,
      feature_branch: goalState.owner_delivery.feature_branch,
      owners: goalState.owner_delivery.owners,
      frozen_at: goalState.completed_at,
    };
...
function ownerBindGoalCommand(goalArgument        , goalStateArgument        , planArgument        , stateArgument        , registryArgument        , featureBranch        )       {
  ...
  const payload = withStateLock(goalStatePath, () => withStateLock(statePath, () => withStateLock(registryPath, () => {
    const goal = parseGoal(readJson(goalPath)); const goalState = parseGoalState(readJson(goalStatePath), goal);
    const { plan } = loadPlanAndState(planPath, statePath); if (goalState.active_plan_path !== planPath) fail("owner-bind-goal plan is not active");
    if (goalState.owner_delivery !== null) fail("goal already has owner_delivery");
    const registry = loadRegistryLocked(registryPath); if (resolve(registry.workspace_root) !== resolve(goal.workspace.root)) fail("registry workspace_root mismatch");
    const currentBranch = gitOutput(goal.workspace.root, ["branch", "--show-current"], "feature branch lookup").trim();
    ...
    for (const ownerId of requiredOwnerIds) {
      ...
      owners[ownerId] = { owner_id: ownerId, worktree_path: "", owner_branch: "",
        base_oid: baseOid, committed_oid: null, committed_at: null,
        merged_oid: null, merged_at: null, status: "pending", owned_modules_glob: owner.owned_modules };
    }
    goalState.owner_delivery = { contract: "OWNER_DELIVERY_V1", registry_ref: registryPath, registry_digest: digestFile(registryPath),
      feature_branch: featureBranch, base_oid: baseOid, required_owner_ids: requiredOwnerIds, owners, pending_intent: null, frozen_evidence: null };
    writeJson(goalStatePath, goalState); return { status: "bound", required_owner_ids: requiredOwnerIds, owner_delivery: goalState.owner_delivery };
  })));
  process.stdout.write(`${JSON.stringify(payload)}\n`);
}
...
function ownerDeliveryReconcileCommand(goalArgument        , goalStateArgument        , planArgument        , stateArgument        , registryArgument        )       {
  ...
  const planPath = resolve(planArgument); const statePath = canonicalPath(statePathFor(planPath), stateArgument, "state path"); const registryPath = resolve(registryArgument);
  const payload = withStateLock(goalStatePath, () => withStateLock(statePath, () => withStateLock(registryPath, () => {
    const goal = parseGoal(readJson(goalPath), false); const goalState = parseGoalState(readJson(goalStatePath), goal);
    const { plan, state } = loadPlanAndState(planPath, statePath, { allowSourceDrift: true }); const delivery = goalState.owner_delivery;
    if (delivery === null || delivery.registry_ref !== registryPath) fail("goal is not bound to registry");
    const registry = loadRegistryLocked(registryPath);
    const currentBranch = gitOutput(goal.workspace.root, ["branch", "--show-current"], "owner delivery feature branch lookup").trim();
    if (currentBranch !== delivery.feature_branch) fail(`main workspace must be on owner delivery feature branch ${delivery.feature_branch}`);
    const featureTip = gitOutput(goal.workspace.root, ["rev-parse", "--verify", delivery.feature_branch], "owner delivery feature branch").trim();
    const liveRequiredOwnerIds = uniqueStrings(plan.tasks.filter((task) =>
      task.role === "work" && task.writable_paths.length > 0 && state.tasks[task.id].status !== "superseded",
    ).map((task) => task.owner_id));
    for (const ownerId of liveRequiredOwnerIds) {
      if (delivery.required_owner_ids.includes(ownerId)) continue;
      const owner = findActiveOwner(registry, ownerId);
      delivery.required_owner_ids.push(ownerId);
      delivery.owners[ownerId] = { owner_id: ownerId, worktree_path: "", owner_branch: "",
    ...
    for (const ownerId of delivery.required_owner_ids) {
      const owner = findActiveOwner(registry, ownerId); const binding = owner.worktree_binding; const entry = delivery.owners[ownerId];
      if (serializedJson(owner.owned_modules) !== serializedJson(entry.owned_modules_glob)) fail(`owner delivery scope drift for owner ${ownerId}`);
      if (binding === null || binding.feature_branch !== delivery.feature_branch || binding.base_oid !== entry.base_oid) fail(`owner delivery binding drift for owner ${ownerId}`);
      if (entry.status === "pending") {
        if (binding.status !== "active" || binding.committed_oid !== null || binding.merged_oid !== null) fail(`owner ${ownerId} pending delivery requires a fresh active worktree`);
      } else if (binding.worktree_path !== entry.worktree_path || binding.owner_branch !== entry.owner_branch) {
        fail(`owner delivery worktree identity drift for owner ${ownerId}`);
      }
      if (entry.committed_oid !== null && binding.committed_oid !== entry.committed_oid) fail(`owner delivery committed_oid drift for owner ${ownerId}`);
      if (entry.merged_oid !== null && binding.merged_oid !== entry.merged_oid) fail(`owner delivery merged_oid drift for owner ${ownerId}`);
      if (binding.status === "active") {
        if (entry.status !== "pending" && entry.status !== "active") fail(`owner delivery status regressed for owner ${ownerId}`);
        if (binding.committed_oid !== null || binding.merged_oid !== null) fail(`owner ${ownerId} active binding carries delivery OIDs`);
        entry.status = "active";
      } else if (binding.status === "sealed") {
        if (entry.status === "merged") fail(`owner delivery status regressed for owner ${ownerId}`);
        if (binding.committed_oid === null || binding.merged_oid !== null) fail(`owner ${ownerId} sealed binding OIDs are invalid`);
        const ownerTip = gitOutput(goal.workspace.root, ["rev-parse", "--verify", binding.owner_branch], "owner delivery branch tip").trim();
        if (ownerTip !== binding.committed_oid) fail(`owner ${ownerId} committed_oid does not equal owner branch tip`);
        entry.status = "sealed";
      } else if (binding.status === "merged" || binding.status === "removed") {
        if (entry.status !== "sealed" && entry.status !== "merged") fail(`owner ${ownerId} cannot enter merged delivery from ${entry.status}`);
        if (binding.committed_oid === null || binding.merged_oid === null) fail(`owner ${ownerId} merged binding OIDs are missing`);
        if (entry.status === "sealed" && entry.committed_oid !== binding.committed_oid) fail(`owner delivery committed_oid changed during merge for owner ${ownerId}`);
        isAncestor(binding.committed_oid, binding.merged_oid, `owner ${ownerId} committed_oid is not contained by merged_oid`);
        isAncestor(binding.merged_oid, featureTip, `owner ${ownerId} merged_oid is not in feature branch`);
        entry.status = "merged";
      } else {
        fail(`owner ${ownerId} has unsupported worktree status ${binding.status}`);
      }
      entry.worktree_path = binding.worktree_path; entry.owner_branch = binding.owner_branch;
      entry.committed_oid = binding.committed_oid; entry.committed_at = binding.committed_at; entry.merged_oid = binding.merged_oid; entry.merged_at = binding.merged_at;
    }
    delivery.registry_digest = digestFile(registryPath); delivery.pending_intent = null; writeJson(goalStatePath, goalState);
    return { status: "reconciled", owner_delivery: delivery };
  })));
  process.stdout.write(`${JSON.stringify(payload)}\n`);
}
...
function ownerAddCommand(
  ...
  const payload = withStateLock(registryPath, () => {
    ...
    const inputDigest = digestFile(ownerDefPath);
    const registry = loadRegistryLocked(registryPath);
    const input = parseRegistryOwnerInput(readJson(ownerDefPath), 0, "owner_def");
    if (registry.owners.some((owner) => owner.owner_id === input.owner_id)) fail(`owner already exists: ${input.owner_id}`);
    const newOwner = registryOwnerFromInput(input, "created", "owner-add");
    const nextOwners = [...registry.owners, newOwner];
    assertRegistryDisjoint(nextOwners.filter((owner) => owner.lifecycle === "active"), "owner-add");
    assertOwnerDependencyGraph(nextOwners, "owner-add");
    const proposal = {
      would_add: { owner_id: newOwner.owner_id, functional_domain: newOwner.functional_domain,
        owned_modules: newOwner.owned_modules, interfaces: newOwner.interfaces,
        depends_on_owners: newOwner.depends_on_owners },
      new_owners: [newOwner.owner_id], registry_version: registry.registry_version,
    };
...
function ownerSplitCommand(
  ...
  const payload = withStateLock(registryPath, () => {
    ...
    const nextParent                = {
      ...
      depends_on_owners: retainedModules.length === 0 ? [] : parent.depends_on_owners,
    };
    stampHistory(nextParent, "split", reason, { child_ids: childIds });
    if (nextParent.lifecycle === "retired") stampHistory(nextParent, "retired", reason, { child_ids: childIds });
    const others = registry.owners.filter((_, index) => index !== parentIndex);
    const nextOwners = [...others, ...children, nextParent];
    ensureUnique(nextOwners.map((owner) => owner.owner_id), "registry owner_id after split");
    assertRegistryDisjoint(nextOwners.filter((owner) => owner.lifecycle === "active"), "owner-split");
    assertOwnerDependencyGraph(nextOwners, "owner-split");
    const proposal = {
      parent_owner_id: parentOwnerId,
      parent_would_lifecycle: nextParent.lifecycle,
      parent_would_retain: retainedModules,
      new_owners: children.map((child) => ({ owner_id: child.owner_id,
        functional_domain: child.functional_domain, owned_modules: child.owned_modules,
        interfaces: child.interfaces, depends_on_owners: child.depends_on_owners })),
    };
    const registryDigest = digestFile(registryPath);
...
function sparseCheckoutDirs(modules          )           {
  ...
  if (dirs.size === 0) fail("owned modules have no safe sparse checkout projection");
  return [...dirs].sort(compareStableStrings);
}

function ownerGitIntentPath(registryPath        )         {
...
function parseOwnerGitIntent(value         , registryPath        )                 {
  ...
  const oid = (value         , label        , nullable = false)                => {
  ...
  const intent                 = {
    ...
    base_oid: oid(source.base_oid, "owner git intent.base_oid")          ,
    pre_owner_oid: oid(source.pre_owner_oid, "owner git intent.pre_owner_oid", true),
    pre_feature_oid: oid(source.pre_feature_oid, "owner git intent.pre_feature_oid")          ,
...
function prepareOwnerGitIntent(registryPath        , intent                )       {
  const path = ownerGitIntentPath(registryPath);
...
function publishOwnerGitRegistry(registryPath        , registry                )       {
...
function ownerOperationMarker(intent                )         {
  return `Goal-DAG-Operation: ${intent.intent_id}`;
}

function commitHasOwnerOperationMarker(workspaceRoot        , oid        , intent                )          {
...
function applyOwnerGitRecovery(registryPath        , intent                , action        )       {
...
function ownerDeliveryRecoverCommand(registryArgument        , mode                    , confirmation               )       {
  const registryPath = resolve(registryArgument);
  const payload = withStateLock(registryPath, () => {
    const intent = loadOwnerGitIntent(registryPath);
...
function worktreeCreateCommand(
  ...
  const payload = withStateLock(registryPath, () => {
    ...
    const baseOid = gitOutput(workspaceRoot, ["rev-parse", "--verify", featureBranch], "feature branch oid").trim();
    const branchSuffix = createHash("sha256").update(`${featureBranch}\0${ownerId}\0${baseOid}`).digest("hex").slice(0, 10);
...
function worktreeCommitCommand(registryArgument        , ownerId        )       {
  ...
  const payload = withStateLock(registryPath, () => {
    ...
    const owner = findActiveOwner(registry, ownerId);
    const binding = owner.worktree_binding;
    if (binding === null || (binding.status !== "active" && binding.status !== "sealed")) fail(`owner ${ownerId} has no active worktree to commit`);
    const worktree = binding.worktree_path;
    const branch = gitOutput(worktree, ["branch", "--show-current"], "owner branch lookup").trim();
    if (branch !== binding.owner_branch) fail(`owner worktree branch mismatch: expected ${binding.owner_branch}, got ${branch}`);
    if (binding.base_oid === null) fail(`owner ${ownerId} worktree binding has no base_oid`);
    const baseAncestor = spawnSync("git", ["-C", worktree, "merge-base", "--is-ancestor", binding.base_oid, "HEAD"], { encoding: "utf8", shell: false });
    if (baseAncestor.status !== 0) fail(`owner ${ownerId} branch no longer descends from base_oid`);
    const dirty = gitOutput(worktree, ["status", "--porcelain=v1", "-z", "--untracked-files=all", "--no-renames"], "owner worktree status")
      .split("\0").filter(Boolean).map((record) => normalizePathPattern(record.slice(3)));
    const committedPaths = binding.base_oid === null ? [] : gitOutput(worktree,
      ["diff", "--name-only", "--no-renames", "-z", `${binding.base_oid}...HEAD`], "owner committed scope diff")
...
function worktreeMergeBackCommand(
  ...
  const payload = withStateLock(registryPath, () => {
    ...
    const owner = findActiveOwner(registry, ownerId);
    const binding = owner.worktree_binding;
    if (binding === null || binding.status !== "sealed" || binding.committed_oid === null) {
      fail(`owner ${ownerId} must be committed/sealed before merge`);
    }
    if (binding.feature_branch !== featureBranch) {
      fail(`feature_branch mismatch: binding has ${binding.feature_branch}, got ${featureBranch}`);
    }
    const workspaceRoot = registry.workspace_root;
    const currentBranch = gitOutput(workspaceRoot, ["branch", "--show-current"], "feature branch lookup").trim();
    if (currentBranch !== featureBranch) fail(`main workspace must already be on feature branch ${featureBranch}`);
    if (gitOutput(binding.worktree_path, ["status", "--porcelain=v1", "-z", "--untracked-files=all", "--no-renames"], "verify owner worktree clean") !== "") fail(`owner ${ownerId} worktree must be clean before merge`);
    const ownerBranch = binding.owner_branch;
    const ownerTip = gitOutput(workspaceRoot, ["rev-parse", "--verify", ownerBranch], "owner branch tip").trim();
    if (ownerTip !== binding.committed_oid) fail(`owner branch tip does not equal committed_oid`);
    if (binding.base_oid === null) fail(`owner ${ownerId} worktree binding has no base_oid`);
    const baseAncestor = spawnSync("git", ["-C", workspaceRoot, "merge-base", "--is-ancestor", binding.base_oid, binding.committed_oid], { encoding: "utf8", shell: false });
    if (baseAncestor.status !== 0) fail(`owner ${ownerId} committed_oid does not descend from base_oid`);
    const diffRaw = gitOutput(
      workspaceRoot,
      ["diff", "--name-only", "--no-renames", "-z", `${featureBranch}...${ownerBranch}`],
      "owner scope diff",
    );
...
function main(argv          )       {
  ...
  if (command === "owner-add" && args.length === 3 && args[2] === "--plan") return ownerAddCommand(args[0], args[1], "plan", null);
  if (command === "owner-add" && args.length === 4 && args[2] === "--confirm") return ownerAddCommand(args[0], args[1], "confirm", args[3]);
  if (command === "owner-query" && args.length === 2) {
    return ownerQueryCommand(args[0], args[1]);
  }
  if (command === "owner-split" && args.length === 4 && args[3] === "--plan") return ownerSplitCommand(args[0], args[1], args[2], "plan", null);
  if (command === "owner-split" && args.length === 5 && args[3] === "--confirm") return ownerSplitCommand(args[0], args[1], args[2], "confirm", args[4]);
  ...
  if (command === "worktree-commit" && args.length === 2) return worktreeCommitCommand(args[0], args[1]);
