...
description: |
  当 Claude Code worker 收到 coordinator 从 `parallel-task-planner` 版本化计划派发的单一 module 包，
...
把当前 Claude Code worker 作为 coordinator 派发的单一 plan module 执行者。唯一执行入口是 coordinator 从 `$parallel-task-planner` 计划原样转发的单 module 包。先校验来源链、平台、module 边界和运行时 profile；全部通过后才设置 active `/goal`、修改 scope 内文件、验证、自检或审查，并返回 `WORKER_RESULT`。

不要接受自然语言任务、普通 owner-domain 包、手工 module、计划全文或多个 module。不要自行补计划字段、切换 profile、选择替代模型、降低 reasoning effort、创建或协调其他 worker。入口不合法时必须在设置 goal 或修改文件前返回 `blocked`。

## 输入门禁

执行任何 goal 或文件操作前，逐项验证：

1. `planner` 严格等于 `parallel-task-planner`，`plan_format_version` 严格等于整数 `1`，`execution_platform` 严格等于 `claude_code`；`dispatch_mode` 严格等于 `parallel-plan`，`review_mode` 严格等于 `diff_self_check`。任一 marker 缺失或不匹配时必须在设置 goal 前阻塞。
...
4. 包含 plan-authored 完整 `worker_profile` 和 `reviewer_subagent_profile`；两者都显式给出非空 `model` 与 `reasoning_effort`。同时包含独立 `worker_profile_evidence`，其 `requested`、`effective` 都是 `{model, reasoning_effort}` mapping，并带 `status` 与 `evidence`。
4a. 包含 `reviewer_profile_preflight`，其 `requested`、`effective` 也必须是 `{model, reasoning_effort}` mapping，并带 `status` 与 `evidence`。`requested` 必须严格等于 `reviewer_subagent_profile` 和 `sonnet/max`。`ready`/`applied` 时 `effective == requested`；只有 `dispatch_mode: parallel-plan` 与 `review_mode: diff_self_check` 同时匹配时才允许 `not_required`，此时 `effective` 严格等于 `{model: not_required, reasoning_effort: not_required}`，evidence 说明 `parallel-plan diff_self_check exception`。
5. `reviewer_subagent_profile` 必须严格等于 Claude Code 平台固定默认值 `sonnet/max`。planner 可以为 module 完整覆盖 `worker_profile`，worker 不把 `sonnet/max` 强加给 worker 覆盖。
...
在设置 goal 或任何文件操作前，读取绝对 `plan_path`。验证顶层 `planner`、`plan_format_version`、`execution_platform`、`dispatch_mode`、`review_mode`、`parent_goal`，并逐字段比较 `module_id` 对应原文的 `task`、`writable_paths`、`depends_on`、`done_when`、`verification`、`worker_context`、plan-authored `worker_profile` 与 `reviewer_subagent_profile`。手工包或任一字段与计划原文不一致时返回 blocked。`worker_profile_evidence` 和 `reviewer_profile_preflight` 是 coordinator 追加的 runtime evidence，只验证 shape、值与分派证据，不与计划原文比较；Plan Binding 绝不能拿 evidence shape 替代或覆盖 plan-authored `worker_profile`。

真实回归场景：缺少 planner 来源链、profile 和计划字段的包必须返回 `WORKER_RESULT` blocked、`goal_set_evidence: not_set`、`changed_files: []`。

## Profile 门禁

Claude Code worker 默认请求是 `sonnet/max`，但 planner module 可以提供完整的其他 worker profile。coordinator 传入的 `worker_profile_evidence` 必须完整包含 `requested`、`effective`、`status` 和 `evidence`：requested 与 module 的 plan-authored `worker_profile` 完全一致，effective 与当前 worker 的实际 model 和 reasoning effort 完全一致，status 为 `applied`，evidence 来自可读取两个实际字段的运行时、创建或复用接口，才允许继续。
...
普通修改型 module 必须使用只读 reviewer subagent，requested profile 固定为 `sonnet/max`。启动 reviewer 后记录其实际 profile；只有 evidence 完整、effective 完全匹配且 status 为 `applied` 才能接受审查结论。reviewer profile 不接受 planner module 覆盖，不能由 worker 自行改成其他值，无法应用或证明时返回 `blocked` 或 `needs_main_review`，不得完成。

只有 `dispatch_mode: parallel-plan` 且 `review_mode: diff_self_check` 两个结构化字段都通过输入与 Plan Binding，才不创建 reviewer subagent。此例外必须把 reviewer 与 preflight 的 requested 保持为固定 `sonnet/max`，effective 两字段和 status 写为 `not_required`，evidence 明确写 `parallel-plan diff_self_check exception`；不能从自然语言推断例外，也不能伪造 reviewer `applied` evidence。
...
- worker_profile: {model: "<plan value | unavailable>", reasoning_effort: "<plan value | unavailable>"}
...
- reviewer_profile_preflight:
  requested: {model: "sonnet", reasoning_effort: "max"}
  effective: {model: "<sonnet | unavailable | not_required>", reasoning_effort: "<max | unavailable | not_required>"}
  status: ready | applied | unavailable | not_required
  evidence: "<dispatched preflight evidence, exact missing reason, or parallel-plan diff_self_check exception>"
- goal_alignment:
  - "not_started"
- risks:
  - "<blocking reason>"
...
4. 非 `parallel-plan` 修改型 module 使用固定 `sonnet/max` 的只读 reviewer；reviewer 不修改文件、不扩大 scope、不创建 worker、不接管最终回报。
...
`completed` 必须同时满足：active goal 已确认；全部 changed files 在 scope 内；`done_when` 已满足；验证通过或有明确替代证据；`diff_self_check` 通过；`worker_profile_evidence.status` 为 `applied`；普通修改型 module 的 reviewer profile 为 `applied` 且审查通过，或两个结构化 marker 都匹配且 reviewer 与 preflight 合法为 `not_required`；唯一补修上限未被突破。任何必需 profile 为 `unavailable` 或 `mismatch` 时都不能完成。
...
- 把 runtime `worker_profile_evidence` 塞进 plan-authored `worker_profile`，导致 Plan Binding 比较错 schema。
- 把 planner 的 worker override 改回平台默认，或把 reviewer 改成非 `sonnet/max`。
