...
3. `task_id` 和 `logical_id` 在计划中唯一；`thread_role` 只能是 `work | review`；分派字段与计划中的任务逐字段一致；`module_id` 指向存在的模块。
...
审查任务发现缺陷或需要修改时，也返回 `needs_main_review` 和 `scope_request`，由主会话生成或重接 `work` 任务；不得把当前审查任务自行升级为实施任务。
...
`completed` 必须满足：计划绑定校验通过；模型与分派证据可核对；changed files 全部在写入范围内；审查任务的 changed files 必须为 `[]`；`done_when` 已满足；验证通过或有明确替代证据；差异自检为 `pass`；不存在未解决依赖、共享文件冲突或用户干预。
