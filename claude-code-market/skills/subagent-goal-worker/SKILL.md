...
Claude Code binding 的 runtime profile 为 `null`，由平台选择执行配置；Codex 固定 `gpt-5.6-sol/high`。这是有意的平台差异。每次接收 binding 时读取 [references/templates.md](references/templates.md) 与 [永久 Owner 治理](../subagent-coordination/references/owner-governance.md)。
