...
description: 当 Claude Code 子代理收到 subagent 模式的 v3 单任务绑定包，需要在当前 parent_goal 的固定 module+role 下执行 work、review 或 verify、自检并原子写入结构化结果时使用；不再委派，不创建执行线程，也不要求模型或思考强度。
---
