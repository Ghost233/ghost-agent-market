...
description: |
  当用户要求把自然语言需求或已有计划文档拆成 Claude Code 并发模块、整理依赖 DAG、判断并发安全性，
...
同时确认 `execution_platform`，并读取可选的顶层 `worker_defaults` 与 module 级 `worker_profile` 覆盖。Claude Code 计划的 `execution_platform` 必须是 `claude_code`；未显式覆盖时，worker 默认使用 `sonnet/max`。
...
3. `model` 与 `reasoning_effort` 都必须是非空、当前 Agent/agent team 调度接口可识别的值。Claude Code 使用 Agent 支持的 `sonnet`/`opus` alias；effort 使用平台支持的显式参数或继承的 `max` session 配置。显式空值、无法解析的值、平台不匹配或解析后缺少任一字段时，将 `safety.status` 写为 `needs_user_review` 并说明具体字段。
...
1. 至少存在两个可执行模块，并且至少一个 batch 含两个以上可同时执行的 module。
2. 同一 batch 中的 `writable_paths` 不存在精确、父子或 glob 相交，且不存在未归属的共享契约。
3. 依赖图无环，`dispatch.batches` 是合法拓扑分层；依赖 module 必须位于更早 batch。有依赖本身不等于 `sequential_only`。
4. 每个模块都有明确 `done_when` 和定向 `verification`。
5. 并行验证不会竞争同一可写产物或共享环境；会竞争的验证必须串行。
6. 全部模块合起来覆盖父目标的完成条件。
7. 来源、格式版本、执行平台、`dispatch_mode: parallel-plan`、`review_mode: diff_self_check` 以及每个 module 的完整 `worker_profile` 均完整且可解析。

只有拓扑和共享写范围使每个 batch 宽度都只能为 `1`、无法形成任何并发 batch 时，才写 `sequential_only` 并保存计划，不自动交接。含依赖但至少一个合法 batch 可并发时仍写 `parallel_safe`，严格按拓扑 batch 执行。范围、依赖或共享契约有证据不足时，写 `needs_user_review`，说明缺少何种确认；绝不自动降级为“看起来能并发”。
...
最终回复必须说明：计划路径、格式版本、执行平台、`dispatch_mode`、`review_mode`、`safety.status`、模块及其完整 worker profiles、拓扑 batch、自动分派是否发生、未分派原因和仍需用户确认的事项。自动分派后以 coordinator 返回的 `PARALLEL_PLAN_RESULT` 判断父目标状态；不得把“计划已生成”表述为父目标已经完成。
