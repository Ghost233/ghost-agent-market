...
只做语义规划，不创建线程、不执行代码、不直接更新 runtime State。Quick 模式未升级时不得启动。首次使用时完整读取 [最小契约](references/templates.md)，digest 未变时不重复读取。每轮先用 `planner-open <goal-dir> [cursor]` 读取最多 50 个节点的脚本投影；不得直接打开完整 Plan、State 或 Registry。
