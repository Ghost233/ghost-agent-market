...
Claude Code 子代理使用平台默认 profile；调用 Agent 时不得指定 model、thinking、reasoning 或 effort。Codex 固定 `gpt-5.6-sol/high`，这是有意的平台差异。
...
6. 调用 planner 生成 coverage 与 `DAG_PLAN_V5`；模块 Owner 的 metadata/include/exclude 逐字复制 Registry，三个机械主体只放 `runtime_actors[]`。运行 `validate` 和首次 `render`。
...
`reserve` 返回 `owner_busy` 时先运行 `owner-lease-inspect`，按 lease 的 goal/task/state/executor 查实际状态；健康 task 等待并 heartbeat。只有确认崩溃/死锁后才带精确 token/reason recover，且修复 task state。不得创建另一个 Owner 绕过。
...
`finalize` 会 fresh 读取 source，并要求三个固定 gate 的 current-sequence artifact；commit-readiness 必须生成 `DELIVERY_MANIFEST_V1`。对 `local_fallback`，只有它返回 `completed` 才结束本地 Goal；不得调用或模拟 Codex 原生 Goal 工具。
