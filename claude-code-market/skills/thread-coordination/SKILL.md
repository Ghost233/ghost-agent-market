...
对当前 batch 中依赖已满足的每个 module，先生成唯一、非空的 `coordinator_assignment_id`，再在同一次 Agent 或 agent team 原子调用中传入完整 module 包和 profile 参数。调度调用必须显式携带计划中的 model；effort 使用平台支持的显式参数，或由可验证的 `CLAUDE_EFFORT=max` / 等价 session 证据继承。Agent/team 返回的 task id 在调用完成后由 coordinator 单独记录，不是首包的前置字段。
...
worker_profile_evidence:
  ...
  evidence: <coordinator assignment id、原子 Agent/team 调用和 effort 证据>
```
