...
- `create_thread`：从 task `module_id` 读取 module profile/context，用 Agent 或 agent-team 创建新 worker，并记录平台返回的 worker id。
- `reuse_thread`：只向 action 指定的原 worker 分派新 task，不自行挑选其他 worker。

平台成功后，把 worker id 原样写入统一 `thread_id` 字段：
