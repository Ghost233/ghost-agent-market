...
短提示必须逐字使用 runtime 输出，不携带计划、DAG、Owner Capsule 或 worker prompt；下一轮从 `.ghost-agent-workflow/` 恢复当前状态。启动前必须由用户明确选择模式；Quick 不启动 Supervisor；DAG 的 `gpt-5.6-luna/medium` Supervisor 最多调度 8 个 ready 线程。配置包含四组 profile，机械 gate 与定向验证由脚本执行。
