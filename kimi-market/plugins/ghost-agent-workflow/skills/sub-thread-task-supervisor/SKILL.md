...
description: 仅供 sub-thread-coordination 在 DAG worktree 创建任务监督子线程时使用。必须在 Planner 前启动持续监督循环，等待 Main 已登记的 Planner、Reviewer、Owner 与 Review 线程，并在终态、挂死或需要调度时通知新 Main；Quick 不得启动。
---

# 任务监督子线程

> 平台差异：Kimi Code 不提供 Codex 原生 Goal 工具，因此使用同一线程内的持续监督 turn；不得伪造原生 Goal 状态。
...
- `wait`：一次最多八项交给宿主 `wait_threads(timeoutMs=60000)`，逐项沿用脚本 cursor；用对应 action id ack 后立即再次运行 `supervisor-next`。超时或仍为 running 也必须 ack。
...
- `notify`：只把脚本提供的 task/thread/status/result_ref/summary 发给 Main，成功后 ack，再继续循环。用户可见文本不显示 `result_ref`。
- `stalled`：脚本累计三次无 cursor 变化后才通知 Main并 ack；不关闭、不 reclaim，等待用户决定。
