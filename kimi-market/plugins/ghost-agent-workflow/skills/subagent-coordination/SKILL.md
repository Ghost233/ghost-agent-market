...
```text
/goal 每轮调用 subagent-coordination skill，完整执行 `./plan.md`。
```
...
Kimi 子代理使用平台默认 profile；调用 Agent 时不得指定 model、thinking、reasoning 或 effort。Codex 固定 `gpt-5.6-sol/high`，这是有意的平台差异。
...
2. 合并仓库强制策略、计划验收和用户追加要求。固定加入 required gate `source-coverage-audit`、`diff-scope-audit` 与 `commit-readiness`；用户只能增加 gate 或授权副作用，不能移除强制项。
...
6. 以 `<goal.json绝对路径>` 调用 planner 生成 coverage 与 `DAG_PLAN_V5`；模块 Owner 的 metadata/include/exclude 逐字复制 Registry，三个机械主体只放 `runtime_actors[]`。运行 `validate` 和首次 `render`。
7. 用户明确只规划时返回 coverage 与 DAG；否则进入执行循环。
...
- 每次 `apply-delta` 后运行 `validate`，默认只展示 DAG diff（新增、替换、失效、保留、依赖与原因）；用户明确要求或 delta 无法唯一解释拓扑时才重新展示完整 `render`。
...
模块 binding 必须含 Registry/Capsule、include/exclude read/search/write、Owner artifact directory、dependency inputs、workspace sequence 与 reusable evidence。严格执行时必须交给真实 allowlist gateway/OS sandbox；不能硬隔离时 fail closed。三个 runtime actor 不是永久 Owner。

`reserve` 返回 `owner_busy` 时先运行 lease inspect，按 goal/task/state/executor 查实际状态；健康 task 等待并 heartbeat。只有确认崩溃/死锁后才带精确 token/reason recover，且修复 task state。不得创建另一个 Owner 绕过。
...
node ${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs owner-lease-inspect <workspace_root> <owner_id>
node ${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs owner-lease-heartbeat <workspace_root> <owner_id> <reservation_token>
node ${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs owner-lease-recover <workspace_root> <owner_id> <reservation_token> <reason>
```

- `spawn_executor`：把 binding 的 `executor_spawn_name` 原样作为 Agent 调用的 `description`，以 `Agent(subagent_type: "coder", run_in_background: true)` 创建后台子代理，`prompt` 为固定分发前缀加完整 binding 原文；worker 需要 Write/Bash/Skill 工具，不得使用 explore/plan 类型，不创建启动握手回合，不指定 model 或思考参数。取得 agent id 后立即 `bind`。固定分发前缀逐字为：`你是 subagent-goal-worker 执行器。立即通过 Skill 工具调用 subagent-goal-worker skill 加载执行协议，并严格按协议处理以下 TASK_BINDING_V5：`
...
`finish` 根据 bind snapshot 自动归因 changed files，有修改即递增 `workspace_change_seq`；验证证据只在同一 sequence 复用。`needs_repair` 仅请求同 Owner 精确路径且无变化时，运行 `expand-task-scope` 验证 Registry 并以新 attempt 重排；其他 Owner/未归属路径走路由或治理。

```text
node ${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs expand-task-scope <plan_path> <state_path> <task_id> <reservation_token> <exact_repo_path>...
```
...
`finalize` 会 fresh 读取 source，并要求三个固定 gate 的 current-sequence artifact；commit-readiness 必须生成 `DELIVERY_MANIFEST_V1`。对 `local_fallback`，只有它返回 `completed` 才结束本地 Goal；不得调用或模拟原生 Goal 工具（CreateGoal/UpdateGoal/SetGoalBudget）。
