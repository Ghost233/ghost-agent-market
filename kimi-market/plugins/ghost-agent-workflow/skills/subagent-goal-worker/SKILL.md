...
whenToUse: 仅当 subagent-coordination 通过 Agent 工具分发完整 TASK_BINDING_V5、binding 原文作为 prompt 且 executor_mode 为 subagent 时使用；普通请求或绕过 coordinator 的调用不得触发。
---
...
Kimi binding 的 runtime profile 为 `null`，由平台默认配置执行；Codex 固定 `gpt-5.6-sol/high`。这是有意的平台差异。每次接收 binding 时读取 [references/templates.md](references/templates.md) 与 [永久 Owner 治理](../subagent-coordination/references/owner-governance.md)。
...
所有 driver 调用固定使用 `node ${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs <command> ...`，不能使用 Codex/Claude 的插件根占位符。
