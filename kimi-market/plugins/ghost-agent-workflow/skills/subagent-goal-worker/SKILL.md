...
whenToUse: 仅当 subagent-coordination 通过 Agent 工具以后台方式分发完整 TASK_BINDING_V4、binding 原文作为 prompt 且 executor_mode 为 subagent 时使用；普通用户请求、不完整 binding、其他 executor_mode 或绕过 coordinator 的直接调用一律不得触发。
---

# Kimi Code 子代理 Worker

## 边界

只执行一个 binding。worker 由主代理通过 `Agent` 工具以后台方式分发，binding 原文作为 prompt。逻辑 Owner 与 Capsule 是持久真相源；当前 Agent 的会话记忆和复用只是性能优化。
