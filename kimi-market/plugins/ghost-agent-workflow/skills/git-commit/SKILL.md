...
whenToUse: |
  用户明确输入 `/skill:git-commit`、要求“提交代码”“提交当前改动”或“commit these changes”时使用；
...
2. 运行 `node ${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs delivery-validate <delivery-manifest.json>`。只有 `status: valid` 才继续；它会重新核对 worktree、HEAD、Registry digest、`workspace_change_seq`、diff-scope evidence、Owner attestations、敏感/runtime/未归属路径和 `git diff --check`。
