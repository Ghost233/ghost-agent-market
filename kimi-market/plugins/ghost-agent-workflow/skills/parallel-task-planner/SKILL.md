...
whenToUse: 仅当 subagent-coordination coordinator 在已验证 GOAL_CONTRACT_V1 下需要创建或局部修订 coverage/DAG 时内部调用；用户直接提出的规划请求、需求未收口、文档审阅或 worker task 已绑定时不得使用。
---
...
本 skill 是 Kimi Code `subagent-coordination` 自动调用的内部能力，不是公开入口。Kimi plan 的 Owner profile 为 `null`（平台默认），Codex 固定 `gpt-5.6-sol/medium` 是有意的平台差异。
...
```text
node "${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs" validate <absolute-plan.json>
node "${KIMI_SKILL_DIR}/../../scripts/goal-dag.mjs" render <absolute-plan.json>
```
