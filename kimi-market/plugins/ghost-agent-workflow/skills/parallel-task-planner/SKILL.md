...
whenToUse: 仅供 subagent-coordination 在已冻结 Goal 下创建 PLAN_COVERAGE_V1、DAG_PLAN_V5 或 DAG_DELTA_V1；普通用户规划、文档审阅或 worker task 不得触发。
---
...
本 skill 是 Kimi `subagent-coordination` 自动调用的内部能力，不是公开入口。Kimi plan 的 profile 为 `null`，由平台默认配置执行；Codex 固定为 `gpt-5.6-sol/high`。这是有意的平台差异。
