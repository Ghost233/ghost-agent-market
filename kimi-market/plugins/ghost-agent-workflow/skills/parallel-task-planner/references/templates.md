...
模块 Owner 必须逐字来自 Registry include/exclude；同模块全部 mode 复用同一 Owner，三个机械主体只放 `runtime_actors[]`。平台差异：Kimi profile 为 `null`，Codex 才固定 `gpt-5.6-sol/high`。work 依赖 source audit，最终依次执行 Owner attestation、diff audit 与 commit readiness。
