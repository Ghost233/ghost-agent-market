...
| --- | --- | --- | --- | --- | --- |
| L1 | 公开 | 专家定义 schema（expert.md）、skills/（只读） | 0644 | 无特殊管控，可两端同步分发 | 不强制 |
...
- 本系统为纯本地插件，无公网在线安装通道；专家定义经 Git + AGENTS.md 同步分发，交互全程本地完成，不经网络（§6.4.2）。
...
- **数据导出管控**：专家定义经 AGENTS.md 两端同步（受控）；私有 handoff 禁止跨专家导出；审计导出需治理方权限 + 水印（操作人 + 时间）。
...
| SSRF | 本系统无公网外部安装通道，SSRF 风险面收敛为本地文件系统访问；若引入外部 URL 拉取须做域名白名单 + 内网 IP 禁访校验 |
| 文件上传 | skill / submodule 加载做魔数 + 扩展名白名单 + 大小限制；外部依赖仅经 AGENTS.md 同步分发与 git submodule，禁本地路径绕过 |
...
- **第三方服务接入**：本系统无第三方在线服务接入；外部凭据（如适用）经 OS Keychain，不落配置。
- **镜像 / 仓库**：N/A（非容器化）；npm 源使用内网镜像 + 白名单，防投毒。
- **供应商管理**：本系统无外部供应商协作；专家定义分发受 AGENTS.md 同步规则约束。
...
- **CLB / Ingress**：不适用；本系统无公网入站通道（§3.2.1）。
...
| 密钥类型 | 本地插件对应 | 存储 / 管理方式 |
| --- | --- | --- |
| 数据库密码 / 第三方 AppKey | 第三方 AppKey（如 外部服务凭据） | OS Keychain（macOS）/ 0600 文件（Linux） |
| 云 AKSK（COS、MQ 等） | 无公有云 AKSK；外部服务凭据（如 安装凭据） | OS Keychain 托管 + 进程隔离读取 |
...
| Q3 | 一致 | 直接引用 D5："本地插件规范，经两端 market 分发，非 SaaS、非多租户" → 无个人信息处理、无跨境，与用户原始诉求（本地插件分发）一致 |
