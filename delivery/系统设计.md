...
| 未启用 | §5 部署架构（§5.1~§5.5） | 本系统为本地插件，按两端 market 分发，无中心化云端部署 / 集群 / 多 AZ；部署与发布形态由《部署设计》文档承接，整节略过 |
...
- **读者对象**：架构师 / 插件工程师（专家·技能设计者）/ 治理·审计方 / 两端同步维护者 / 评审委员会。
...
| 现有架构知识库（内部对标） | goal-dag v5 spec（五条不变量 / progress.json / events.jsonl）、owner-governance.md（ACL / handoff / Capsule / propose-current-approve-apply / route）、sub-thread-coordination.skill、sub-thread-goal-worker.skill、sub-thread-task-supervisor.skill、AGENTS.md（两端同步规则） | 第 3 章模块设计、第 7 章安全边界 | 本文档自带（内部权威源文件，逐条引用） |
...
| --- | --- | --- | --- |
| BD-1 专家定义域 | 编写专家身份 / scope ACL / 技能挂载 / 模型 profile / 长期线程归属 | 专家·技能设计者、两端同步维护者 | 上游供给 BD-2 编排；写入 BD-3 registry |
...
| C-04 | 外部（Git 分发 / AGENTS.md） | BC-ExpertSpec | Conformist | 顺从 AGENTS.md 同步规则 + Git 同步分发 | 外部不可控的强上游（AGENTS.md 同步规则），本端完全遵循其模型 |
...
| P-03 | 对接外部不可控系统（外部 / AGENTS.md）：必须建 Conformist 适配，禁止把外部模型贯穿到本域 |
...
| F10 | 两端同步 | AGENTS.md 规则落地 CC/Codex | BD-1 | 完整实现 |
...
| 角色 | 两端同步维护者 | 使用 | 按 AGENTS.md 同步 CC/Codex |
| 上游复用底座 | E-01 goal-dag.mjs | 推送状态 → 本系统 | 调度 / 生命周期 / 不变量 |
| 上游复用底座 | E-02 AGENTS.md 同步基线 | 约束 → 本系统 | 两端同步 / Git 身份 |
| 外部协作 | E-03 Git 远程分发 | 本系统 → 拉取 | 专家定义同步分发 |
...
| 层次 | 内容 | 数据来源 |
| --- | --- | --- |
| 接入层 | 设计者工作台 / 端用户 CLI / 治理审计端视图 / 两端同步 | §2.3 用例图角色 |
...
| 集成对接层 | goal-dag.mjs / AGENTS.md / SkillOpt / owner-registry 迁移源 | §3.1.3 E-xx |
...
| E-02 | AGENTS.md 同步基线 | 平台组 | 同步规则 | 双向 | 文档约定 | v1（内部） | 两端同步 / Git 身份 | @平台组 |
| E-03 | Git 远程分发 | 各端 | 安装通道 | 消费 | Git/SSH | v1 | 专家定义同步分发 | @平台组 |
...
| --- | --- | --- | --- |
| M1 专家定义规范 | E-02, E-03, E-04 | C-02 | E-02 同步规则缺失 → 两端定义不一致 |
...
| 文件存储 | 本地文件系统 + JSON | — | 选型理由：本地插件无集中式 DB（§0 声明 §4 略过）；JSON 人类可读、可被 git 跟踪与两端同步；不选 SQLite 以增加嵌入式 DB 依赖 |
...
专家定义规范模块覆盖「编写专家身份 / scope ACL / 技能挂载 / 模型 profile / 长期线程归属」业务流程（F1~F5），业务逻辑在设计者、registry、编排运行时之间流动，涉及设计者工作台、expert-registry（M2）、goal-dag（M3）、AGENTS.md 同步（E-02）、安装（E-03）等内外部系统。
...
| 同步 | SYNC | `expert sync {expert_id}` | 触发两端同步校验 | — | SyncResultVO | 是（expert_id+端） |
...
| 数据导出与共享 | 专家定义经 AGENTS.md 两端同步；私有 handoff 禁止跨专家导出 |
