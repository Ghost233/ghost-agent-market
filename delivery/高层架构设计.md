...
ghost-agent-market 是覆盖三端的插件市场，现有 goal-dag v5 子线程体系基于 Main/Owner/Worker/Supervisor/Reviewer 五角色与 owner-registry 治理。触发事件：用户要求新增 workbuddy 专家团，将 Owner 与 Worker 合并为专家并收敛 owner 职责，评估可行性与冲突。本期定位：一套专家(Expert)与技能(Skill)设计规范 + 专家团编排模型，作为 goal-dag v5 演进，复用 Main/Supervisor/Review 元角色与 ACL/handoff 治理。
...
| D4 | MVP / 完整版边界 | MVP 覆盖专家定义规范 + expert-registry 迁移 + 跨专家 handoff 隔离 + Review 保留 + **Dashboard 完整设计（含专职 Dashboard 专家持有后台进程）**；多专家并行编排 / ZCode 单端差异化延后至完整版 | §4.3 / §4.5 |
...
| 受影响方：同步维护者 | 三端同步维护者 | 按 AGENTS.md 同步 CC / Codex / ZCode | 规范变更能正确同步到三端、单端差异可声明 | AGENTS.md 同步规则 / ZCode 在线连接规则 |

最终用户角色（专家 / 技能设计者、workbuddy 端用户）与受影响方角色（治理 / 审计方、三端同步维护者）的关注点均需在专家定义规范与编排模型中体现。
...
| P2 | 通用 skill（supervisor / coordination）与专家执行语义未统一抽象，新增端（workbuddy）难以直接复用现有子线程体系 | 三端（CC / Codex / ZCode）已存在 AGENTS.md 同步规则，但专家执行语义未标准化，新端接入需重复理解 Owner + Worker 两套契约 | 最终用户 A / B | 中 | P2 |
...
| 分发基线 | AGENTS.md（三端同步规则 / ZCode 在线连接 / Git 身份） | 作为专家规范分发与同步基线 | AGENTS.md | ZCode 单端差异经演进说明管理 |
...
| B3 现有 sub-thread 协调模式 | 本框架内部（sub-thread-coordination + ZCode role agent 模板） | 唯一协调入口 / 单 Main / Supervisor wait-notify / install-agents 安装通道 | 硬边界、可恢复、跨端安装 | 内部机制复用 | 中 |
...
- **部分借鉴**：B3（sub-thread 协调模式）—— 借鉴点（建议）：作为专家团协调骨架，Main / Supervisor 保留为元角色，ZCode agent 模板复用为专家安装通道。
...
| 多租户 | 否（插件规范，按端分发；ZCode 单端差异经 AGENTS.md 同步规则与独立演进说明管理） |
| 服务对象 | 专家 / 技能设计者（插件工程师）、workbuddy 端用户、三端同步维护者、治理 / 审计方 |
| 上游依赖 | goal-dag.mjs（调度 / 生命周期 / 不变量）、AGENTS.md（同步 / Git 身份）、SkillOpt |
| 下游消费者 | workbuddy 端运行实例、ZCode role agent 安装（install-agents.py）、Dashboard 专家（持有后台进程，只读消费 progress.json / events.jsonl）+ Dashboard 可视化 |
...
| --- | --- | --- | --- | --- | --- |
| MVP | W1 ~ W6 | F1 ~ F10（专家定义规范 / expert-registry 迁移 / 跨专家 handoff 隔离 / Review 保留 / 通用 skill 沿用 / 三端同步）+ F13（Dashboard 完整设计）+ F14（Dashboard 专家 持有后台进程） | 单 Main / Supervisor 骨架沿用；expert-registry 复用 owner-registry 机制（改名 + 字段合并）；本地插件分发；Dashboard 专家持有后台进程（长驻，轮询 progress.json / events.jsonl 只读入口） | 跑通「定义专家 → 编排专家团 → 跨专家 handoff → Review」主链路；三端同步无回归；跨专家 handoff 审计 100%；Dashboard 实时呈现专家团状态与 handoff 审计视图 | F11 ~ F12（多专家并行编排调度优化 / ZCode 单端深度差异化） |
| 完整版 | W7 ~ W12 | + F11 ~ F12 | 多专家并行编排、ZCode 单端差异化经独立演进说明、跨端版本校验（Dashboard / Dashboard 专家已在 MVP 交付，完整版增强视图与告警） | 多专家并行编排稳定运行；三端规范一致性与单端差异均可验证 | — |
...
- MVP 中可延后的能力（多专家并行编排 / ZCode 单端差异化）在本节明确「完整版替换计划」，不影响 MVP 主链路。
...
**遗漏点识别**：
1. ZCode 单端差异化（install-agents.py / 独立演进说明）需在专家规范中预留「单端差异声明」字段，避免三端强一致导致 ZCode 无法单端优化。
2. 三端同步规则（AGENTS.md）需在专家定义变更时触发同步校验，防止一端改专家定义而另两端滞后。
...
| 上游 | AGENTS.md 同步规则 | 平台组 | 三端同步 / Git 身份 / ZCode 在线连接 | 文档约定（编辑时遵循） | 同步 | 改专家定义须触发三端同步校验 |
...
| 下游 | ZCode role agent 安装 | ZCode 端 | install-agents.py 安装专家 | 在线 GitHub raw | 同步 | 仅在线 marketplace 连接，禁本地路径 |
...
**运营主线**：每日（设计者审专家定义合规性）→ 每周（治理方审跨专家 handoff 审计完整性）→ 每月（三端同步维护者校验 CC / Codex / ZCode 规范一致性）。
...
F10. 三端同步（AGENTS.md 规则落地：CC / Codex / ZCode）
...
> **范围变更说明（G3 修订）**：Dashboard 可视化（F13）与 Dashboard 专家（F14）已于本修订由「完整版延后」纳入 **In-Scope（MVP）**，不再作为 Out-of-Scope / 延后项；多专家并行编排（F11）、ZCode 单端差异化（F12）仍为完整版延后项。
...
| O3 | Out-of-Scope | SaaS / 云端多租户部署 | 本系统为本地插件规范，按三端 market 分发 | 不做 |
...
| AGENTS 三端同步基线 | 基础能力层 | 平台组 | — | 三端分发 | 是 |
...
| F10 | 三端同步 | AGENTS 规则落地 | CC / Codex / ZCode 同步专家定义变更；ZCode 单端差异经演进说明 | P1 | ✅ | ✅ | V3 |
| F11 | 专家团编排 | 多专家并行 | 多专家并行编排调度优化（extension of MVP 单链） | P2 | ❌ | ✅ | V3 |
| F12 | ZCode 差异化 | 单端演进说明 | ZCode 单端差异独立声明与演进 | P2 | ❌ | ✅ | — |
...
| 三端同步一致性面板 | CC / Codex / ZCode 规范一致性 | 实时刷新 / 一键校验 | ✅ |
