...
主线程是唯一 Git 写入者。子代理只读分析并返回提交建议；不得让子代理暂存、提交、修改文件或继续委派。主分析固定使用 `git_commit_worker` 的 `gpt-5.3-codex-spark/high`，只有主分析发生工具、系统或契约失败时才允许一次 `gpt-5.6-luna/medium` fallback。提交顺序是硬约束：先从最深层脏 submodule 向外提交，再提交主工程中的 submodule 指针和其他改动。
...
3. 分析包使用中文，包含仓库绝对路径、用户授权范围和显示名称 `[GA][审查][执行] Git 提交分析`，要求子代理：
...
4. 使用 `wait_agent` 等待该子代理完成。主分析正常返回有效契约时不得创建第二个分析代理。
5. 只有以下任一情况出现时，才创建一次 fallback 分析线程：主分析 `spawn_agent` 真实调用失败；代理发生系统、工具或模型终止失败；返回内容缺少完整 `GIT_COMMIT_ANALYSIS_V1` 契约。`spawn_agent` 当前不支持 `gpt-5.6-luna`，不得尝试用普通子代理伪造该 fallback。先用 `list_projects` 按仓库绝对路径唯一解析 project id，再调用 `create_thread`，固定传入：

```text
target: {type: project, projectId: <resolved>, environment: {type: local}}
model: "gpt-5.6-luna"
thinking: "medium"
prompt: <沿用原只读分析包，增加一次性 fallback 标记>
```
...
fallback 必须沿用同一仓库、授权范围、只读要求和结果契约，不得修改文件、继续委派或再次 fallback。主分析返回合法 `status: "blocked"` 表示真实安全边界，不触发 fallback。fallback 创建、读取或契约校验失败时不得静默降级为主线程直接提交。

6. 最终采用的子代理必须返回一个 `GIT_COMMIT_ANALYSIS_V1` 对象，至少包含：

```json
{
  "contract": "GIT_COMMIT_ANALYSIS_V1",
  "status": "ready | blocked",
  "profile_evidence": "git_commit_worker:gpt-5.3-codex-spark/high | create_thread:gpt-5.6-luna/medium fallback",
  "repository": "<仓库绝对路径>",
