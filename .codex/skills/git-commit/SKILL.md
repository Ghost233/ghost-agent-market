...
本实现仅用于 Codex App，依赖其 collaboration 子代理和可见 fallback 线程能力。在本仓库提交此 skill 时，这项平台依赖就是 `AGENTS.md` 所要求的单端实现理由；只校验项目测试版与 Codex marketplace 源同步。

在当前 checkout 中提交用户授权的现有改动。保持用户改动，不创建 worktree，不切换分支，不 push，不改写历史。

主线程是唯一 Git 写入者。子代理只读分析并返回提交建议；不得让子代理暂存、提交、修改文件或继续委派。主分析先使用继承当前运行时 profile 的通用子代理。`gpt-5.6-luna/medium` 不是并行分析、第二意见或一般错误兜底；只有运行时错误证据明确表明当前继承的模型 profile 不可创建或不可运行时，才允许一次 fallback。提交顺序是硬约束：先从最深层脏 submodule 向外提交，再提交主工程中的 submodule 指针和其他改动。
...
通用子代理继承当前运行时 profile；主分析调用不得传入 `model`、`reasoning_effort`、`reasoning.summary`、`service_tier` 或 `priority`。不得通过 `functions.exec`、`ALL_TOOLS` 或工具搜索预判 profile 是否可用，必须以一次真实 `spawn_agent` 调用结果为准。

3. 分析包使用中文，包含仓库绝对路径、用户授权范围、显示名称 `[GA][审查][执行] Git 提交分析`，以及第 6 步的完整 `GIT_COMMIT_ANALYSIS_V1` 字段契约。由于 `fork_turns: "none"`，不得假设子代理能读取本 skill；必须在 `message` 中明确要求主分析的 `profile_evidence` 精确等于 `spawn_agent:default/inherited`，并要求子代理：
   ...
   - 最终只返回一个符合契约的 JSON 对象，不把 Luna 或两个 profile 写入正常的主分析结果。
4. 使用 `wait_agent` 等待该子代理完成。主分析正常返回有效契约时不得创建第二个分析执行单元，包括返回合法 `status: "blocked"` 的情况。若已经返回实质分析、但只缺少契约字段或 JSON 格式不合法，只允许通过 `followup_task` 向同一子代理发送一次纯格式修复请求；不得重新分析、创建新代理或改用 Luna。修复后仍无效时停止并报告原始结果。
5. 只在以下任一错误被真实工具结果明确证明时，才创建一次 fallback 分析线程：
   - `spawn_agent` 明确报告通用子代理继承的当前模型 profile 不可用、不受支持或无法创建。
   - 子代理终态明确报告当前继承的模型 profile 无法初始化、不可用或不受支持，因而未能开始分析。

超时、通信中断、普通工具错误、Git 或仓库错误、契约缺失或格式错误、安全阻塞，以及无法归因到当前继承 profile 的系统失败，都不得触发 fallback；应在任何 Git 写操作前停止并报告原始证据。不得根据猜测或预检结果判定 profile 不可用。

确认满足上述唯一门禁后，才使用 Luna 接管同一次只读分析。不得为了切换模型而新建第二个普通子代理伪造该 fallback。先用 `list_projects` 按仓库绝对路径唯一解析 project id，再调用 `create_thread`，固定传入：
...
创建后取当前环境的本地时间，按 `YYYYMMDD-HHmm` 格式生成到分钟的时间戳，再用 `set_thread_title` 设置 `[时间戳] git-commit · Luna 模型回退分析`，例如 `[20260720-1435] git-commit · Luna 模型回退分析`。标题中的“模型回退”仅表示分析 profile 从当前继承 profile 回退到 Luna，不表示撤销、回滚或重做任何 Git 提交。然后低频调用 `read_thread(includeOutputs: true)`，直到取得终态 `GIT_COMMIT_ANALYSIS_V1`；最多读取十二轮，首轮立即读取，后续轮次各等待 10 秒。该线程保持可见，不自动归档。
...
主分析成功时，`profile_evidence` 必须精确等于 `spawn_agent:default/inherited`；只有实际发生 fallback 时才改为 `create_thread:gpt-5.6-luna/medium fallback`。不得使用 `|`、不得同时报告两个 profile，也不得把 Luna 写进正常的主分析结果。
...
- 最终使用的 `spawn_agent:default/inherited` 或 `gpt-5.6-luna/medium fallback` profile evidence，以及分析警告。
