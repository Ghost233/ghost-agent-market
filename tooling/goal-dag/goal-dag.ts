...
type GoalController = "codex_native" | "standalone_thread" | "local_fallback";
...
type ThreadWorkflowConfig = {
  parallel: number;
  profiles: Record<ThreadProfileRole, WorkerProfile>;
};
...
type ExecutionSubject = OwnerDefinition | RuntimeActorDefinition | ReviewSubjectDefinition;
...
type TaskNodeType = "leaf" | "composite";
...
type TaskDefinition = {
  ...
  runtime_actor_id: RuntimeActorId | null;
  task: string;
  depends_on: string[];
  writable_paths: string[];
  ...
  review_batch_key: string | null;
  review_blocks_dependents: boolean;
  review_reasons: string[];
  reviews_task_ids: string[];
...
type PublishedOwnerArtifact = {
  contract: "OWNER_INTERFACE_V1" | "OWNER_HANDOFF_V1" | "COMMIT_ATTESTATION_V1";
  ref: string;
  digest: string;
  audience: string[];
};
...
type WorkerResultV5 = {
  ...
  reviewed_results: Array<{
    task_id: string;
    result_ref: string;
    result_digest: string;
  }>;
  review_plan_digest: string | null;
  review_workspace_digest: string | null;
  review_upgrade_reason?: string | null;
  ...
  published_artifacts: PublishedOwnerArtifact[];
};
...
type TaskState = {
  ...
  task_baseline_ref: string | null;
  task_baseline_digest: string | null;
  expanded_writable_paths: string[];
  accepted_change_seq: number | null;
};
...
type RunState = {
  ...
  runtime_actors: Record<string, OwnerState>;
  reviewers: Record<string, OwnerState>;
...
const RUNTIME_ACTOR_IDS = new Set<RuntimeActorId>([
  "source-audit",
  "diff-audit",
  "commit-readiness",
]);

function taskSubjectId(task: TaskDefinition): string {
...
function isModuleTask(task: TaskDefinition): boolean {
  return task.owner_id !== null;
}

function executionSubjects(plan: Plan): ExecutionSubject[] {
  return [
    ...plan.owners,
    ...plan.runtime_actors,
...
function subjectForTask(plan: Plan, task: TaskDefinition): ExecutionSubject {
  if (task.role === "review") {
    return {
      id: taskSubjectId(task),
      role: "review",
...
function subjectStateForTask(state: RunState, task: TaskDefinition): OwnerState {
  const id = taskSubjectId(task);
...
function subjectScope(subject: ExecutionSubject): string[] {
  return "writable_paths" in subject ? subject.writable_paths : [];
}

function ownerAllowsPath(owner: OwnerDefinition, path: string): boolean {
...
function subjectReusePolicy(subject: ExecutionSubject): "owner_affinity" | "ephemeral" {
  return "reuse_policy" in subject ? subject.reuse_policy : "ephemeral";
}

function isOwnerDefinition(subject: ExecutionSubject): subject is OwnerDefinition {
...
function requireChineseText(value: unknown, label: string): string {
...
function compactUserSummary(value: string): string {
...
function compactThreadSummary(value: string): string {
  const normalized = value
    .replace(/[\u0000-\u001f\u007f]/gu, " ")
    .replace(/\s+/gu, " ")
    .trim();
...
function requireParallelCount(value: unknown, label: string): number {
...
function threadWorkflowConfigPath(workspaceRoot: string): string {
...
function parseThreadWorkflowConfig(value: unknown): ThreadWorkflowConfig {
  ...
  const profiles = {} as Record<ThreadProfileRole, WorkerProfile>;
...
function defaultThreadWorkflowConfigValue(): Record<string, unknown> {
  return {
...
function normalizeThreadWorkflowConfig(value: unknown): {
...
function readThreadWorkflowConfig(workspaceRoot: string): ThreadWorkflowConfig {
...
function loadThreadWorkflowConfig(workspaceRoot: string): ThreadWorkflowConfig {
...
function refreshProgressDocumentsForMutation(paths: string[], strict = false): void {
...
function ownerRegistryPathFor(workspaceRoot: string): string {
  return join(resolve(workspaceRoot), ".ghost-agent-workflow", "owners", "registry.json");
}

function persistentOwnerCapsulePathFor(workspaceRoot: string, ownerId: string): string {
...
function ownerLeasePathFor(workspaceRoot: string, ownerId: string): string {
  return join(
    resolve(workspaceRoot),
    ".ghost-agent-workflow",
    "runtime",
...
function parseOwnerLease(value: unknown, expectedOwnerId?: string): OwnerLeaseV1 {
...
function acquireOwnerLease(
...
): { acquired: boolean; lease: OwnerLeaseV1 } {
  ...
  return withStateLock(leasePath, () => {
    if (existsSync(leasePath)) {
      const existing = parseOwnerLease(readJson(leasePath), task.owner_id as string);
      if (
...
function updateOwnerLease(
  ...
  task: TaskDefinition,
  reservationToken: string,
  update: { executor_id?: string | null; status?: "reserved" | "running" },
): OwnerLeaseV1 | null {
  ...
  return withStateLock(leasePath, () => {
    ...
    const lease = parseOwnerLease(readJson(leasePath), task.owner_id as string);
...
function approvedOwnerRegistry(goal: GoalContract): {
  ref: string;
  digest: string;
  revision: number;
  owners: ApprovedOwnerSnapshot[];
} {
...
function validatePlanOwnersAgainstRegistry(
  plan: Plan,
  goal: GoalContract,
  ownerValidationTaskIds?: Set<string>,
): void {
  const ownerIdsInScope = ownerValidationTaskIds === undefined
    ? new Set(plan.tasks.map((task) => task.owner_id).filter((value): value is string => value !== null))
    : new Set(plan.tasks
...
function fixedGoalGates(): GoalGate[] {
  return [
    {
...
function goalCreateCommand(targetArgument: string, workspaceArgument: string): void {
  ...
  for (const field of ["allow_deploy", "allow_external_write"] as const) {
...
function parseRuntimeActor(value: unknown, index: number): RuntimeActorDefinition {
  ...
  const id = requireIdentifier(source.id, `runtime_actors[${index}].id`) as RuntimeActorId;
...
function parseTaskSubgraph(value: unknown, taskId: string, index: number): TaskSubgraph | null {
...
function parseTask(value: unknown, index: number): TaskDefinition {
  ...
  const runtimeActorId = runtimeActorIdRaw as RuntimeActorId | null;
  ...
  return {
    ...
    review_policy: reviewPolicy as ReviewPolicy,
    ...
    node_type: nodeTypeRaw as TaskNodeType,
    parent_task_id: parentTaskId,
    subgraph,
  };
}

function fixedRuntimeActors(): RuntimeActorDefinition[] {
  return [
    {
      id: "source-audit",
      role: "verify",
...
function parsePlanInputItem(value: unknown, index: number): RequiredPlanItem {
...
function expandPlanInputTask(
  value: unknown,
  index: number,
  defaults: { parent_task_id?: string | null; owner_id?: string | null } = {},
): TaskDefinition {
  ...
  const actor = input.actor === undefined || input.actor === null
    ? null
    : requireIdentifier(input.actor, `plan input.tasks[${index}].actor`) as RuntimeActorId;
  if (actor !== null && !RUNTIME_ACTOR_IDS.has(actor)) {
    fail(`plan input.tasks[${index}].actor is invalid: ${actor}`);
  }
  const inferredRole: TaskRole = reviews.length > 0 ? "review" : actor !== null ? "verify" : "work";
  const role = input.role === undefined
    ? inferredRole
    : requireString(input.role, `plan input.tasks[${index}].role`) as TaskRole;
  ...
  const risk = (input.risk ?? (role === "work" ? "medium" : "low")) as RiskLevel;
  if (!new Set<RiskLevel>(["low", "medium", "high"]).has(risk)) {
    fail(`plan input.tasks[${index}].risk is invalid: ${String(input.risk)}`);
  }
  const defaultReview: ReviewPolicy = role === "work" && risk === "high" ? "immediate" : "none";
  const review = (input.review ?? defaultReview) as ReviewPolicy;
  if (!new Set<ReviewPolicy>(["batch", "immediate", "final_only", "none"]).has(review)) {
  ...
  const coverageEffect: CoverageEffect = role === "work"
    ? "implementation"
...
function addFixedRuntimeTasks(
  ...
  const runtimeTask = (
    ...
    logicalId: string,
    title: string,
    actor: "source-audit" | "diff-audit" | "commit-readiness",
    task: string,
    after: string[],
    gate: string,
    done: string,
  ): TaskDefinition => parseTask({
    id,
...
function buildPlanDraft(
  ...
  revision: number,
): { plan: Plan; coverage: PlanCoverage; planDigest: string } {
  ...
  const safetyStatus = (input.safety ?? "parallel_safe") as SafetyStatus;
...
function planCreateCommand(goalArgument: string, planArgument: string): void {
...
type PlannerReview = {
  ...
  parallelism: "pass" | "revise";
  too_complex: boolean;
  too_simple: boolean;
  changes: string[];
};

function plannerReviewDirectory(planPath: string): string {
  return join(dirname(planPath), "planner-reviews");
}

function plannerReviewContextPath(planPath: string, revision: number): string {
  return join(plannerReviewDirectory(planPath), `context-${revision}.json`);
}

function plannerReviewPath(planPath: string, revision: number): string {
  return join(plannerReviewDirectory(planPath), `review-${revision}.json`);
}

function plannerReviewMetrics(plan: Plan, configuredParallel: number): Record<string, unknown> {
...
function buildPlannerReviewContext(planPath: string, plan: Plan): Record<string, unknown> {
...
function parsePlannerReview(value: unknown, planPath: string, plan: Plan): PlannerReview {
...
function acceptedPlannerReview(planPath: string, plan: Plan): PlannerReview {
...
function plannerReviewSubmitCommand(planArgument: string): void {
  const planPath = resolve(planArgument);
  if (existsSync(statePathFor(planPath))) fail("planner review is only allowed before Plan activation");
  const { plan } = parsePlan(readJson(planPath), planPath);
...
function planReviseCommand(goalArgument: string, planArgument: string): void {
  const goalPath = resolve(goalArgument);
  const planPath = resolve(planArgument);
...
type ParsePlanOptions = {
  ...
  ownerValidationTaskIds?: Set<string>;
  skipOwnerRegistryValidation?: boolean;
};
...
function ownerValidationTaskIdsFromRawState(value: unknown): Set<string> {
  const state = requireRecord(value, "state");
  const tasks = requireRecord(state.tasks, "state.tasks");
  return new Set(Object.entries(tasks)
    .filter(([, taskValue]) => {
...
function directChildrenFor(taskId: string, plan: Plan): TaskDefinition[] {
  return plan.tasks.filter((task) => task.parent_task_id === taskId);
}

function descendantTaskIds(taskId: string, plan: Plan, visited = new Set<string>()): Set<string> {
...
function buildEffectiveAncestors(plan: Plan): Map<string, Set<string>> {
  const byId = new Map(plan.tasks.map((task) => [task.id, task]));
  const effectiveDependencies = (task: TaskDefinition): string[] => {
    const result = new Set(task.depends_on);
    let parentId = task.parent_task_id;
    const containmentVisited = new Set<string>();
    while (parentId !== null) {
...
function validateTaskHierarchy(plan: Plan): void {
  const byId = new Map(plan.tasks.map((task) => [task.id, task]));
  for (const task of plan.tasks) {
    ...
    const subgraph = task.subgraph as TaskSubgraph;
...
function validateExplicitReviews(
  ...
  const live = (task: TaskDefinition): boolean => liveTaskIds === undefined || liveTaskIds.has(task.id);
...
function validateGraph(
  ...
  const readinessAncestors = ancestors.get(commitReadinessTask.id) ?? new Set<string>();
...
function goalResultPathFor(goalPath: string): string {
  return join(dirname(goalPath), "result.json");
}

function cleanupCompletedGoal(goalPath: string): void {
...
function subgraphRequestPathFor(resultPath: string): string {
...
function parseTaskState(
  ...
  if (result.task_baseline_ref !== null && result.task_baseline_digest !== null) {
    canonicalPath(
      taskBaselinePathFor(planPath, taskId, result.attempt, result.reservation_token as string),
...
function parseSubjectState(
  ...
  planPath: string,
  stateKey: "owners" | "runtime_actors" | "reviewers",
): OwnerState {
...
function initializeState(planPath: string, plan: Plan): RunState {
  ...
  const runtimeActors: Record<string, OwnerState> = Object.fromEntries(
  ...
  const reviewers: Record<string, OwnerState> = Object.fromEntries(
...
function loadPlanAndState(
  planPath: string,
  statePath: string,
  options: { allowSourceDrift?: boolean; allowOwnerRegistryDrift?: boolean } = {},
): {
  plan: Plan;
  goal: GoalContract;
...
function renderCommand(planArgument: string): void {
  ...
  for (const task of tasks) {
    ...
    const hierarchy = task.node_type === "composite"
      ? ` · composite:${(task.subgraph as TaskSubgraph).task_ids.length}`
...
function dependencyResolved(
  ...
  if (task.node_type === "composite") {
    return (task.subgraph as TaskSubgraph).task_ids.every((childId) =>
...
function effectiveTaskStatus(task: TaskDefinition, plan: Plan, state: RunState): TaskStatus {
  if (task.node_type === "leaf") return state.tasks[task.id].status;
  const childStatuses = (task.subgraph as TaskSubgraph).task_ids.map((childId) => {
  ...
  if ((task.subgraph as TaskSubgraph).task_ids.every((childId) =>
...
function taskDepth(task: TaskDefinition, plan: Plan): number {
...
function boundaryDependenciesForTask(task: TaskDefinition, plan: Plan): string[] {
  const byId = new Map(plan.tasks.map((candidate) => [candidate.id, candidate]));
  const result = new Set(task.depends_on);
  let parentId = task.parent_task_id;
  const visited = new Set<string>();
...
function resultTasksForDependency(
  ...
  if (task.node_type === "composite") {
    return (task.subgraph as TaskSubgraph).task_ids.flatMap((childId) =>
...
function executorNameSegment(value: string, fallback: string): string {
  return value
    .toLowerCase()
    .replace(/[^a-z0-9_]/g, "_")
    .replace(/_+/g, "_")
...
function threadTitle(task: TaskDefinition, subject?: ExecutionSubject): string {
  void subject;
...
function goalThreadTitles(goal: GoalContract): Record<string, string> {
...
function compositePlannerThreadTitle(task: TaskDefinition): string {
  return `[GA][任务][子图规划] ${compactThreadSummary(task.title)}`;
}

function acceptedResultUserMessage(task: TaskDefinition, result: WorkerResultV5): string {
...
function effectiveWritablePaths(task: TaskDefinition, taskState: TaskState): string[] {
...
function reviewContextForTask(
  ...
  reviewed_results: Array<{ task_id: string; result_ref: string; result_digest: string }>;
} | null {
...
function taskBinding(
  ...
  const registryBinding = existsSync(registryPath)
    ...
    : null;
  const audit: Record<string, { path: string; contract: string }> = {};
  ...
  return {
    ...
    scope: {
      ...
      exclude: moduleOwner ? (subject as OwnerDefinition).excluded_paths : [],
...
function reserveCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    const ownerBusy: Record<string, unknown>[] = [];
...
function abandonCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    if (task.owner_id !== null && task.role !== "review") {
      const owner = subjectForTask(plan, task) as OwnerDefinition;
      ...
      writeTransaction(statePath, [
        [ownerState.capsule_ref as string, capsule],
        [statePath, state],
      ]);
    } else {
      writeJson(statePath, state);
...
function checkpointCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    capsule.updated_at = new Date().toISOString();
    writeJson(ownerState.capsule_ref as string, capsule);
    return {
      task_id: taskId,
      owner_id: task.owner_id,
      owner_generation: ownerState.generation,
...
function parsePublishedOwnerArtifact(value: unknown, index: number): PublishedOwnerArtifact {
  ...
  return {
    contract: contract as PublishedOwnerArtifact["contract"],
    ref,
    digest,
    audience,
  };
...
function parseWorkerResult(
  ...
  taskState: TaskState,
  allowedPublishedArtifactDirectory?: string,
): WorkerResultV5 {
  ...
  const result: WorkerResultV5 = {
    ...
    diff_self_check: requireString(source.diff_self_check, "worker result.diff_self_check") as WorkerResultV5["diff_self_check"],
...
function expectedDiffScopeAudit(
  ...
  for (const workTask of liveWorkTasks) {
    ...
    const owner = subjectForTask(plan, workTask) as OwnerDefinition;
...
function sourceAuditCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    let classifications: SourceCoverageClassification[];
    if (classificationsArgument === "-") {
      ...
      classifications = sourceBlocks.blocks.map((block) => {
        ...
        return {
          block_id: block.id,
          disposition: "non_requirement" as const,
          plan_item_ids: [],
...
type DeliveryManifestV1 = {
  ...
  diff_scope_ref: string;
  diff_scope_digest: string;
  generated_consistency: "owner_attested";
};

type CommitReadinessAuditV1 = {
  contract: "COMMIT_READINESS_AUDIT_V1";
  audit_task_id: string;
  ...
  delivery_manifest_digest: string;
  conclusion: "commit_ready";
};

function isSensitiveDeliveryPath(path: string): boolean {
...
function ownerAttestationFor(
  ...
  ownerId: string,
  changedFiles: string[],
): { artifact: PublishedOwnerArtifact; commitMessage: string } {
...
function expectedCommitReadiness(
  ...
  task: TaskDefinition,
  taskState: TaskState,
): { audit: CommitReadinessAuditV1; delivery: DeliveryManifestV1 } {
  ...
  const delivery: DeliveryManifestV1 = {
    contract: "DELIVERY_MANIFEST_V1",
    goal_id: goal.goal_id,
    plan_ref: planPath,
    plan_digest: state.plan_digest,
  ...
  const audit: CommitReadinessAuditV1 = {
    contract: "COMMIT_READINESS_AUDIT_V1",
    audit_task_id: task.id,
    runtime_actor_id: "commit-readiness",
    attempt: taskState.attempt,
...
function deliveryValidateCommand(manifestArgument: string): void {
...
function finishCommand(
  ...
  const planPath = resolve(planArgument);
  const statePath = canonicalPath(statePathFor(planPath), stateArgument, "state path");
  let consumedCandidatePath: string | null = null;
  const cleanupAfterFinish: string[] = [];
  const payload = withStateLock(statePath, () => {
    const { plan, goal, coverage, state } = loadPlanAndState(
      planPath,
      statePath,
    ...
    if (isOwnerDefinition(owner)) {
      ...
      updatePersistentOwnerCapsule(
        ...
        owner,
        result,
        taskState.result_digest as string,
      );
      writeTransaction(statePath, [
        [ownerState.capsule_ref as string, capsule],
        [statePath, state],
...
function parseDelta(value: unknown): {
  ...
  owner_transition: null | {
    ...
    task_rebindings: Array<{ task_id: string; owner_id: string }>;
  };
  ...
  const ownerTransition = source.owner_transition === undefined || source.owner_transition === null
    ...
    : (() => {
      ...
      return {
        contract: "OWNER_TRANSITION_V1" as const,
...
function expandDeltaInput(
  ...
  let ownerTransition: Record<string, unknown> | null = null;
...
function verifiedOwnerTransition(
  transition: NonNullable<ReturnType<typeof parseDelta>["owner_transition"]>,
  goal: GoalContract,
  state: RunState,
): ReturnType<typeof approvedOwnerRegistry> {
...
function ownerDefinitionFromApproved(owner: ApprovedOwnerSnapshot): OwnerDefinition {
  return {
    id: owner.id,
    role: "work",
    responsibility: owner.responsibility,
...
function applyDeltaCommand(
  ...
  const planPath = resolve(planArgument);
  const statePath = canonicalPath(statePathFor(planPath), stateArgument, "state path");
  const obsoleteResultRefs: string[] = [];
  const payload = withStateLock(statePath, () => {
    ...
    for (const repair of delta.repairs) {
      ...
      if (isOwnerDefinition(repairedSubject)) {
        const capsule = capsuleWrites.get(repairedSubjectState.capsule_ref as string) ??
          loadOwnerCapsule(
            repairedSubject,
        ...
        capsule.updated_at = new Date().toISOString();
        capsuleWrites.set(repairedSubjectState.capsule_ref as string, capsule);
      }
    }
    for (const disposition of delta.source_dispositions) {
      const taskState = state.tasks[disposition.task_id];
      if (disposition.action === "invalidate") {
        ...
        const capsule = capsuleWrites.get(ownerState.capsule_ref as string) ?? loadOwnerCapsule(
          subject,
          ownerState,
          state.goal_digest,
          state.source_revision,
        ...
        capsule.updated_at = new Date().toISOString();
        capsuleWrites.set(ownerState.capsule_ref as string, capsule);
      } else {
        taskState.validated_source_revision = state.source_revision;
...
function pendingSubgraphRequests(plan: Plan, state: RunState): Array<{
  task_id: string;
  reservation_token: string;
  request_ref: string;
  request_digest: string;
...
function pendingReviewUpgrades(state: RunState): Array<{ task_id: string; reason: string }> {
...
function dashboardSnapshot(planArgument: string, stateArgument: string): Record<string, unknown> {
  const planPath = resolve(planArgument);
  const statePath = canonicalPath(statePathFor(planPath), stateArgument, "state path");
...
function dashboardSnapshotRead(planPath: string, statePath: string): Record<string, unknown> {
  ...
  const { plan, goal, coverage, state } = loadPlanAndState(
    ...
    const drift = sourceDriftPayload(goal, goalState, plan, state);
    const inspection = goalState.status === "completed"
      ? { problems: [] as string[], passed_gates: goal.verification_gates.map((gate) => gate.id) }
      : inspectCompletion(planPath, plan, goal, coverage, state);
...
type ProgressResultObservation = {
  task_id: string;
  title: string;
  status: string;
  reason: string | null;
...
type ProgressObservation = {
  fingerprint: string;
  source: Record<string, unknown>;
  taskStates: ProgressTaskObservation[];
  taskResults: ProgressResultObservation[];
};

type ProgressDocumentRefresh = {
  changed: boolean;
  path: string;
  document: Record<string, unknown>;
};

function progressDocumentPathFor(planPath: string): string {
  return join(dirname(planPath), "progress.json");
}

function progressEventsPathFor(planPath: string): string {
  return join(dirname(planPath), "events.jsonl");
}

function parseProgressEventLine(line: string, label: string): Record<string, unknown> {
  let parsed: unknown;
  try {
...
function readProgressEvents(path: string): Record<string, unknown>[] {
...
function readProgressEventTail(path: string, maxBytes = 256 * 1024): Record<string, unknown>[] {
  if (!existsSync(path)) return [];
...
function readExistingProgressDocument(path: string): Record<string, unknown> | null {
...
function observeProgressSources(planPath: string, statePath: string): ProgressObservation {
  const plan = requireRecord(readJson(planPath), "progress plan");
  const state = requireRecord(readJson(statePath), "progress state");
  const taskDefinitions = new Map<string, Record<string, unknown>>();
...
function publicTaskResult(result: ProgressResultObservation): Record<string, unknown> {
...
function refreshProgressDocument(
  ...
  return withStateLock(documentPath, () => {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      const previousTaskResults = new Map<string, Record<string, unknown>>(
        Array.isArray(previous?.task_results)
          ? previous.task_results.flatMap((value) => {
            ...
            return [[value.task_id, value] as [string, Record<string, unknown>]];
          })
          : [],
      );
      const currentTaskResults = new Map(
        taskResults.map((result) => [result.task_id as string, result]),
      );
      ...
      const previousSnapshotTasks = new Map<string, Record<string, unknown>>(
        isRecord(previous?.snapshot) && Array.isArray(previous.snapshot.tasks)
          ? previous.snapshot.tasks.flatMap((value) => {
            if (!isRecord(value) || typeof value.id !== "string") return [];
            return [[value.id, value] as [string, Record<string, unknown>]];
          })
          : [],
      );
...
function progressDocumentCommand(planArgument: string, stateArgument: string): void {
...
function dashboardSnapshotCommand(planArgument: string, stateArgument: string): void {
...
function dashboardConnectHost(host: string): string {
  if (host === "0.0.0.0") return "127.0.0.1";
  if (host === "::") return "::1";
  return host;
}

function dashboardSourceId(workspace: string, goalId: string): string {
  return createHash("sha256").update(`${workspace}\n${goalId}`).digest("hex").slice(0, 20);
}

function dashboardPathWithin(root: string, candidate: string): boolean {
  const offset = relative(root, candidate);
  return offset === "" || (!offset.startsWith("..") && !isAbsolute(offset));
}

function localDashboardSource(options: DashboardServeOptions): DashboardSource {
...
function dashboardCatalog(sources: Map<string, DashboardSource>, leaderRuntimeId: string | null) {
...
function dashboardSourceFromRequest(
  requestUrl: URL,
  sources: Map<string, DashboardSource>,
...
function dashboardLocalClient(request: IncomingMessage, host: string): boolean {
...
async function readDashboardRequest(request: IncomingMessage): Promise<Record<string, unknown>> {
  const chunks: Buffer[] = [];
  let size = 0;
...
function parseDashboardParticipant(payload: Record<string, unknown>): {
...
function dashboardCommand(args: string[]): void {
  ...
  const sources = new Map<string, DashboardSource>([[localSource.id, localSource]]);
  const liveResponses = new Map<ServerResponse, string>();
  ...
  const sendSourceSnapshot = (source: DashboardSource) => {
    if (!leader) return;
  ...
  let sourceReaper: ReturnType<typeof setInterval> | null = null;
  const shutdown = () => {
  ...
  const announce = (role: "leader" | "participant", actualPort = options.port) => {
    if (announced) return;
    announced = true;
...
function ownerLeaseInspectCommand(workspaceArgument: string, ownerIdArgument: string): void {
  const workspaceRoot = resolve(workspaceArgument);
...
function expandTaskScopeCommand(
  ...
  let obsoleteResultRef: string | null = null;
  const payload = withStateLock(statePath, () => {
    const { plan, goal, state } = loadPlanAndState(planPath, statePath, {
      allowSourceDrift: true,
    });
    ...
    let reopenedCapsule: OwnerCapsule | null = null;
    if (reopenRepair) {
      const previousResultRef = taskState.result_ref as string;
    ...
    if (reopenedCapsule === null) writeJson(statePath, state);
      ...
      [(subjectStateForTask(state, task).capsule_ref as string), reopenedCapsule],
      [statePath, state],
    ]);
    return {
...
function validateTaskSubgraphRequest(
  ...
  task: TaskDefinition,
  taskState: TaskState,
): { reason: string; required_capabilities: string[]; suggested_subtasks: string[] } {
...
function parseTaskSubgraphExpansion(value: unknown): TaskSubgraphExpansion {
...
function expandTaskSubgraphInput(
  ...
  requestPath: string,
  request: ReturnType<typeof validateTaskSubgraphRequest>,
): Record<string, unknown> {
...
function expandSubgraphCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    const expandedParent: TaskDefinition = {
      ...parent,
      node_type: "composite",
      subgraph: {
        contract: "TASK_SUBGRAPH_V1",
    ...
    if (parent.owner_id !== null) {
      const owner = subjectForTask(nextPlan, expandedParent) as OwnerDefinition;
      ...
      capsule.updated_at = new Date().toISOString();
      writes.push([parentSubjectState.capsule_ref as string, capsule]);
    }
...
function readStructuredInput(expectedContract: string): Record<string, unknown> {
...
function validateThreadEndpoint(value: unknown, label: string): void {
...
function validateThreadRegistry(value: Record<string, unknown>): void {
  assertExactFields(
    value,
...
function threadRegistryCommand(action: string, args: string[]): void {
...
function supervisorControlRole(taskId: string): "planner" | "planner_reviewer" {
...
function supervisorControlThreadKey(taskId: string): string {
...
function parseSupervisorActionId(value: string): {
  ...
  const kind = requireString(payload.kind, "supervisor action kind") as SupervisorActionKind;
  if (!new Set<SupervisorActionKind>(["create", "wait", "stalled", "notify"]).has(kind)) {
...
function supervisorPaths(goalDirectoryArgument: string): {
  goalDirectory: string;
  planPath: string;
  statePath: string;
  threadsPath: string;
...
function registeredThreadForExecutor(
  ...
  executorId: string,
): { key: string; thread: Record<string, unknown> } | null {
...
function supervisorResultNotice(
  ...
  task: TaskDefinition,
  taskState: TaskState,
): { result_ref: string | null; summary: string } {
...
function supervisorMainRoute(registry: Record<string, unknown>): {
  thread: string;
  host: string;
} {
...
function supervisorOwnerSyncPrompt(goalDirectory: string, ownerId: string): string {
  const command = [
    "node",
...
function supervisorControlNext(
  ...
  let completedControl: { task: string; attempt: number } | null = null;
  for (;;) {
    ...
    const main = requireRecord(current.main, "Supervisor main route") as {
      thread: string;
      host: string;
    };
...
function supervisorRecordCommand(
  ...
  const receipt = withStateLock(statePath, () => withStateLock(threadsPath, () => {
    ...
    if (event === "created") {
      ...
      validateThreadRegistry(registry);
      writeTransaction(statePath, [
        ...(existsSync(bindingRef) ? [] : [[bindingRef, binding] as [string, unknown]]),
        [statePath, state],
...
function supervisorControlRecord(
  goalDirectoryArgument: string,
  action: { kind: SupervisorActionKind; task: string; attempt: number },
  args: string[],
): Record<string, unknown> {
  const goalDirectory = resolve(goalDirectoryArgument);
  const goal = parseGoal(readJson(join(goalDirectory, "goal.json")));
  ...
  return withStateLock(routesPath, () => withStateLock(threadsPath, () => {
    ...
    if (action.kind === "create") {
      ...
      (registry.watches as Record<string, unknown>[]).push({
        task_id: action.task,
...
function expandEvidenceInput(value: unknown, index: number): Record<string, unknown> {
...
function expandPublishedArtifactInput(value: unknown, index: number): Record<string, unknown> {
  ...
  const contracts: Record<string, PublishedOwnerArtifact["contract"]> = {
...
function expandWorkerResultInput(
  ...
  const status = requireString(input.status, "result input.status") as WorkerTerminalStatus;
...
function runSelfJson(args: string[], input?: Record<string, unknown>): Record<string, unknown> {
...
function taskRunId(taskId: string, taskState: TaskState): string {
...
function readPlainSemanticInput(label: string): string {
  const raw = readFileSync(0, "utf8");
  if (Buffer.byteLength(raw, "utf8") > 16 * 1024) fail(`${label} exceeds 16 KiB`);
  return requireString(raw.trim(), label);
}

function workflowDefinitionPath(goalDirectory: string): string {
  return join(goalDirectory, "workflow.json");
}

function workflowStatePath(goalDirectory: string): string {
  return join(goalDirectory, "workflow-state.json");
}

function workflowRoutesPath(goalDirectory: string): string {
  return join(goalDirectory, "routes.json");
}

function workflowDashboardPath(goalDirectory: string): string {
...
function readWorkflowRoutes(goalDirectory: string): WorkflowRoutesV1 {
...
function workflowRouteReceipt(goalDirectory: string, role: WorkflowThreadRole): Record<string, unknown> | null {
  return readWorkflowRoutes(goalDirectory)[role];
}

function preferredWorkflowThread(
  goalDirectory: string,
  role: WorkflowThreadRole,
): Record<string, unknown> | null {
...
function activeWorkflowDirectories(workspaceRoot: string): string[] {
...
function parseWorkflowDefinition(value: unknown, path: string): WorkflowDefinitionV1 {
...
function parseQuickRun(value: unknown): QuickRunV1 {
...
function parseQuickAccepted(value: unknown): QuickAcceptedV1 {
...
function loadWorkflow(goalDirectoryArgument: string): {
  directory: string;
...
function syntheticQuickGoal(workflow: WorkflowDefinitionV1, definitionPath: string): GoalContract {
  return {
    contract: "GOAL_CONTRACT_V1",
    goal_id: workflow.id,
    execution_platform: EXPECTED_PLATFORM,
...
function currentOwnerThreadPath(workspaceRoot: string, ownerId: string): string {
  return join(workspaceRoot, ".ghost-agent-workflow", "runtime", "owners", ownerId, "thread.json");
}

function currentOwnerThreadIds(workspaceRoot: string): Set<string> {
...
function hasCurrentOwnerChange(workspaceRoot: string): boolean {
  return existsSync(currentOwnerChangePath(workspaceRoot, "request.json"));
}

function currentOwnerChangeDirectory(workspaceRoot: string): string {
  return join(
    resolve(workspaceRoot),
    ".ghost-agent-workflow",
    "runtime",
...
function currentOwnerChangePath(workspaceRoot: string, name: string): string {
  return join(currentOwnerChangeDirectory(workspaceRoot), name);
}

function currentOwnerChangeApplied(workspaceRoot: string): boolean {
...
function preferredOwnerThread(workspaceRoot: string, ownerId: string, generation: number): Record<string, unknown> | null {
...
function quickOwner(
  ...
  definitionPath: string,
  ownerId: string,
): { goal: GoalContract; owner: OwnerDefinition; registry: ReturnType<typeof approvedOwnerRegistry> } {
...
function quickTask(run: QuickRunV1, owner: OwnerDefinition): TaskDefinition {
...
function quickThreadTitle(run: QuickRunV1): string {
...
function quickBinding(
  ...
  run: QuickRunV1,
  owner: OwnerDefinition,
  registry: ReturnType<typeof approvedOwnerRegistry>,
): Record<string, unknown> {
...
function workflowDispatchCommand(goalDirectoryArgument: string, ownerIdArgument: string): void {
...
function workflowReviewCommand(goalDirectoryArgument: string): void {
  const loaded = loadWorkflow(goalDirectoryArgument);
...
function workflowDashboardAckCommand(goalDirectoryArgument: string, statusArgument: string): void {
  const goalDirectory = resolve(goalDirectoryArgument);
...
function workflowSupervisorInitCommand(goalDirectoryArgument: string): void {
  const goalDirectory = resolve(goalDirectoryArgument);
...
function workflowNativeConfirmCommand(goalDirectoryArgument: string, tokenArgument: string): void {
  const goalDirectory = resolve(goalDirectoryArgument);
...
function quickWorkerOpenCommand(goalDirectoryArgument: string, runIdArgument: string): void {
  const loaded = loadWorkflow(goalDirectoryArgument);
...
function quickWorkerOutcomeCommand(
  ...
  const payload = withStateLock(loaded.statePath, () => {
    ...
    const evidence = status === "completed" && run.kind === "work"
      ...
      : [];
    const subject: ExecutionSubject = run.kind === "review"
      ? {
...
function acceptQuickCandidate(
  loaded: ReturnType<typeof loadWorkflow>,
  state: WorkflowStateV1,
): { state: WorkflowStateV1; completed: Record<string, unknown> } {
  const run = state.run ?? fail("quick workflow has no current run");
  const candidatePath = quickRuntimePath(loaded.directory, "candidate.json");
  ...
  if (result.status === "completed" && run.kind === "work") {
    ...
    const accepted: QuickAcceptedV1 = {
      owner: owner.id,
      executor: run.executor as string,
...
function finalizeQuickWorkflow(loaded: ReturnType<typeof loadWorkflow>, state: WorkflowStateV1): Record<string, unknown> {
...
function upgradeQuickWorkflowToDag(loaded: ReturnType<typeof loadWorkflow>, state: WorkflowStateV1): void {
...
function completeWorkflowWrapper(goalDirectory: string, resultRef: string): void {
  const definitionPath = workflowDefinitionPath(goalDirectory);
...
function quickWorkflowStepCommand(goalDirectoryArgument: string): void {
  const loaded = loadWorkflow(goalDirectoryArgument);
...
function workerOpenCommand(goalDirectoryArgument: string, runIdArgument: string): void {
...
function workerVerifyCommand(
  ...
  const compactOutput = (value: string | null | undefined): string => {
    const output = value ?? "";
...
function plannerReviewDecisionCommand(goalDirectoryArgument: string, args: string[]): void {
  const goalDirectory = resolve(goalDirectoryArgument);
  const planPath = join(goalDirectory, "plan.json");
  ...
  if (existsSync(planPath)) {
    ...
    if (state === null) {
      ...
      subgraphRequests = pendingSubgraphRequests(parsed, state).map((request) => {
        const task = parsed.tasks.find((candidate) => candidate.id === request.task_id) as TaskDefinition;
        ...
        : reviewUpgrades.length > 0
          ...
          : subgraphRequests.length > 0
            ...
            : coordinatedNextAction(
              ...
              parsed,
              goal,
              loaded?.coverage as PlanCoverage,
              state,
              goalState,
            );
...
function plannerSubmitCommand(goalDirectoryArgument: string, modeArgument: string, runId?: string): void {
  const goalDirectory = resolve(goalDirectoryArgument);
  const goalPath = join(goalDirectory, "goal.json");
  const planPath = join(goalDirectory, "plan.json");
  const statePath = join(goalDirectory, "state.json");
...
function startWorkflow(
  ...
  mode: WorkflowMode,
  objective: string,
  dagWorktrees: DagWorktreesV1 | null = null,
): Record<string, unknown> {
...
function supervisorRecoverRunCommand(goalDirectoryArgument: string, runIdArgument: string): void {
...
function workflowStepCommand(goalDirectoryArgument: string, deferDelivery = false): void {
  const goalDirectory = resolve(goalDirectoryArgument);
  const definitionPath = workflowDefinitionPath(goalDirectory);
