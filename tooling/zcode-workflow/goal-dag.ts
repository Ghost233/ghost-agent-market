...
function taskLifecycleForWorkerResult(status                      )



  {
  if (status === "completed") return { status: "completed", reason: null, action: null };
  const reason             = status === "blocked" ? "input_missing" : "task_failed";
  return { status: "stopped", reason, action: STOP_ACTION_MATRIX[reason] };
}
const STOP_ACTION_MATRIX                                               = Object.freeze({
...
const STOP_REASONS = new Set            (Object.keys(STOP_ACTION_MATRIX)                );
const REQUIRED_ACTIONS = new Set                (Object.values(STOP_ACTION_MATRIX));
...
function parseStopDirective(
  ...
  const reason = reasonValue === null
    ? null
    : requireString(reasonValue, `${label}.reason`)              ;
  const action = actionValue === null
    ? null
    : requireString(actionValue, `${label}.action`)                  ;
  if (status !== "stopped") {
...
function clearTaskStop(taskState           , status                                )       {
...
function stopWorkflow(state                        , reason            )       {
  state.status = "stopped";
  state.reason = reason;
  state.action = STOP_ACTION_MATRIX[reason];
}

function activateWorkflow(state                        )       {
  state.status = "active";
  state.reason = null;
  state.action = null;
}

function isClaimedTask(taskState           )          {
  return taskState.status === "running" && taskState.executor_id === null;
}

function isBoundRunningTask(taskState           )          {
  return taskState.status === "running" && taskState.executor_id !== null;
}

function isSupersededTask(taskState           )          {
...
function lifecycleContractCommand()       {
...
function normalizeThreadWorkflowConfig(value         )
  ...
  const source = requireRecord(value, "thread workflow config");
  const rawProfiles = requireRecord(source.profiles, "thread workflow config.profiles");
  const originalProfileKeys = Object.keys(rawProfiles).sort().join(",");
  delete rawProfiles.supervisor;
  const profileKeys = Object.keys(rawProfiles).sort().join(",");
  if (!Object.hasOwn(rawProfiles, "main") &&
      profileKeys === "owner,planner,review") {
    rawProfiles.main = {
  ...
  return {
    value: source,
    changed: Object.keys(rawProfiles).sort().join(",") !== originalProfileKeys,
  };
}
...
function gitBranchExists(path        , branch        )          {
  return gitAttempt(path, ["show-ref", "--verify", "--quiet", `refs/heads/${branch}`]).ok;
}

function gitWorktreeForBranch(path        , branch        )                {
...
function gitCommonDirectory(path        )         {
...
function gitAllStatusMap(workspaceRoot        )                      {
...
function commitScriptManagedPaths(worktree        , paths          , message        )         {
...
function parseVerificationDefinition(value         , index        )                         {
...
function parseTaskState(
  ...
  const status             = legacyStatus === "reserved"
    ? "running"
  ...
  const legacyReason                    = legacyStatus === "blocked"
    ? "input_missing"
...
function parseState(
  ...
  const workflowStatus = source.status === undefined
    ? source.goal_refresh_pending === true || source.owner_change !== undefined && source.owner_change !== null
      ? "stopped"
      : "active"
    : requireString(source.status, "state.status")                  ;
  if (!WORKFLOW_STATUSES.has(workflowStatus)) fail("state.status is invalid");
  const legacyStopReason                    = source.goal_refresh_pending === true
    ? "plan_invalid"
...
function ownerIntegrationPath(goalDirectory        , runId        )         {
...
function finishCommand(
  ...
  const payload = withStateLock(statePath, () => {
    ...
    let automaticallyAttributedChanges          ;
    if (integration !== null) {
...
function summarizeState(state          )                         {
  const statuses               = ["pending", "running", "completed", "stopped"];
...
function normalizeSupervisorWaitStatus(turnStatusArgument        )         {
...
function supervisorInspectionReport(title        , conclusion        )         {
  const conclusions                         = {
    thread_ended: "线程已经结束",
...
function supervisionDocumentPath(goalDirectory        )         {
  return join(goalDirectory, "supervision.md");
}

function supervisorGoalObjective(_goal              )         {
...
function refreshSupervisionDocument(goalDirectoryArgument        )         {
  const goalDirectory = resolve(goalDirectoryArgument);
...
function supervisorMainResumeCommand(goalDirectory        )           {
...
function supervisorNextCommand(goalDirectoryArgument        , limitArgument         )       {
  ...
  const computePayload = ()                          => withStateLock(
    statePath,
    ...
    const threads = requireRecord(registry.threads, "thread registry.threads");
    const originalWatches = registry.watches                             ;
...
function publicSupervisorProjection(payload                         )                          {
...
function supervisorPublicProjection(goalDirectoryArgument        )                          {
  const goalDirectory = resolve(goalDirectoryArgument);
...
function supervisorNextFacadeCommand(goalDirectoryArgument        )       {
  process.stdout.write(`${JSON.stringify(supervisorPublicProjection(goalDirectoryArgument))}\n`);
}

function supervisorStopFacadeCommand(goalDirectoryArgument        )       {
  const goalDirectory = resolve(goalDirectoryArgument);
  const payload = runSelfJson([
    "supervisor-next",
    goalDirectory,
  ...
  const onlyMainNotification = projection.action === "notify" &&
    ...
    (projection.items                             ).every((item) => item.kind === "main");
...
function dagWorktreesPath(goalDirectory        )         {
  return join(goalDirectory, "worktrees.json");
}

function parseDagWorktrees(value         , goalDirectory        )                 {
...
function readDagWorktrees(goalDirectory        )                 {
...
function completedWorkflowDirectories(workspaceRoot        )           {
  const root = join(resolve(workspaceRoot), ".ghost-agent-workflow", "runtime", "goals");
  if (!existsSync(root)) return [];
  return readdirSync(root, { withFileTypes: true }).flatMap((entry) => {
    if (!entry.isDirectory() || entry.isSymbolicLink()) return [];
...
function parseWorkflowState(value         )                  {
  ...
  if (!WORKFLOW_STATUSES.has(source.status                  )) fail("workflow state.status is invalid");
  const stop = parseStopDirective(
    source.reason ?? null,
    source.action ?? null,
    source.status                  ,
...
function workflowThreadCommand(
  ...
  if (!new Set                    (["main", "planner", "planner_reviewer", "supervisor"]).has(role)) {
...
function plannerSourceBlocks(goalDirectory        , goal              )                                 {
...
function dagBranchName(developmentKey        )         {
  return `ga/${developmentKey}/main`;
}

function developmentKeyFromDagBranch(branch        )                {
  const match = /^ga\/([a-z0-9][a-z0-9_-]{0,63})\/main$/u.exec(branch);
  return match?.[1] ?? null;
}

function pendingDagBranches(workspace        )           {
  return gitOutput(
    workspace,
...
function ownerBranchName(dagBranch        , ownerId        )         {
...
function gitlinkEntries(worktree        )                 {
  const entries                 = [];
...
function workflowOwnerSyncCommand(goalDirectoryArgument        , ownerIdArgument        )       {
  const goalDirectory = resolve(goalDirectoryArgument);
  const planPath = join(goalDirectory, "plan.json");
  const statePath = join(goalDirectory, "state.json");
  const { plan, goal, state } = loadPlanAndState(planPath, statePath, { allowSourceDrift: true });
...
function gitChangedPathsSince(worktree        , base        )           {
...
function workflowOwnerFinishCommand(goalDirectoryArgument        , runIdArgument        )       {
  const run = taskForRun(goalDirectoryArgument, runIdArgument);
...
function prepareOriginalGitlinks(worktrees                )                          {
...
function mergeDagBackToOriginal(goalDirectory        )                          {
  const worktrees = readDagWorktrees(goalDirectory);
...
function preflightDagCleanup(worktrees                , originalHead        )       {
  const branches = new Set        ();
  const paths = new Set        ([worktrees.original.path]);
  const candidates = [
...
function removeMergedBranch(workspace        , branch        , label        )       {
...
function supervisorResumeCommand(
  ...
  if (isSupervisorControlTask(taskId)) {
    ...
    threadTarget = requireRecord(receipt.target, "control resume target")
    ...
    const resumed = withStateLock(statePath, () => withStateLock(threadsPath, () => {
      ...
      const registry = readThreadRegistry(threadsPath, plan.goal_id);
      const watch = (registry.watches                             ).find((candidate) =>
        candidate.task_id === taskId && candidate.attempt === attempt
